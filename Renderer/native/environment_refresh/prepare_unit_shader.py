...
The selected source material uses authored frames and first-map normal detail.
"""
from pathlib import Path
import hashlib,json
ROOT=Path(__file__).resolve().parents[3];HERE=Path(__file__).resolve().parent
SOURCE=ROOT/'Renderer/lab/shared/shaders/objects/beauty_objects.hlsl'
DETAIL=ROOT/'Renderer/lab/shared/shaders/objects/unit_microfacet.hlsl'
def main():
...
'''
    ...
    (HERE/'unit-shader-provenance.json').write_text(json.dumps({'authority':str(SOURCE.relative_to(ROOT)),
      ...
      'ported':['GGX','sky-weighted ambient','source AO response','source gloss response','emissive response','rim fill','legacy multiplicative owner-color response','positive-alpha modulation','authored tangent normals','RGB dual-lobe specular'],
      ...
      'lean_decode':'first map with authored frame; second-map variance constants unresolved',
      'detail_authority':str(DETAIL.relative_to(ROOT)), 'detail_sha256':hashlib.sha256(DETAIL.read_bytes()).hexdigest()},indent=2)+'\n')
