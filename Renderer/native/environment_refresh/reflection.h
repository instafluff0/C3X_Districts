...
struct Reflection {
    ...
    bool ensure(ID3D11Device*device,std::string const&root,char const*directory="environment_refresh",unsigned extent=136,unsigned height=0){
        ...
        for(unsigned i=0;i<5;i++){
            ...
            drop(v);drop(p);
            if(SUCCEEDED(hr) && i==4){
                hr=render_core::compile_cached(wide.c_str(),"VSReflectionInstance","vs_5_0",&v,&errors);
                if(errors){OutputDebugStringA(static_cast<char const*>(errors->GetBufferPointer()));drop(errors);}
    ...
    void bind(ID3D11DeviceContext*context,unsigned width=0,unsigned height=0,float offset_x=8,float offset_y=8){
        float values[]={height_pixels,depth_metric,2.5f/112.f,enabled?1.f:0.f,float(width?width:native_extent*2),float(height?height:native_height*2),offset_x,offset_y};
