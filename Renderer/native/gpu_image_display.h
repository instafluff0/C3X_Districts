...
float4 ps565(float4 at:SV_Position):SV_Target {return native_color(at,11,63);}
        ...
        context->ClearState();D3D11_VIEWPORT viewport={0,0,float(width),float(height),0,1};context->RSSetViewports(1,&viewport);
        context->RSSetScissorRects(1,&clip);context->RSSetState(raster.Get());context->OMSetRenderTargets(1,&target,nullptr);
        context->IASetPrimitiveTopology(D3D11_PRIMITIVE_TOPOLOGY_TRIANGLELIST);context->VSSetShader(vertex.Get(),nullptr,0);
        context->PSSetShader(pixel[native_format].Get(),nullptr,0);context->PSSetShaderResources(0,1,&input);
        context->Draw(3,0);context->ClearState();return true;
    }
};
}
