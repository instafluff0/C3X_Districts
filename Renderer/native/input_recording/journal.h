...
enum class Kind:std::uint32_t { manifest=1,configuration,scene,world_page,unit,unit_forget,
    camera,native_operation,native_snapshot,image_commands,tactical,visual,presentation,result,reset,footer,asset,settings,native_bridge };
...
public:
    ...
    bool emit(Kind kind,std::uint64_t ticks,Bytes payload,std::uint32_t flags=0)noexcept{
        try{
            ...
            if(kind==Kind::footer||std::uint32_t(kind)<1||std::uint32_t(kind)>std::uint32_t(Kind::native_bridge)){reason=Stop::unsupported;stopping=true;enabled=false;wake.notify_one();return false;}
...
struct SegmentReader {
    ...
    bool next(Event& item){
        ...
        auto kind=header.u32();require(kind>=1&&kind<=std::uint32_t(Kind::native_bridge),"unknown input event");item.kind=Kind(kind);
