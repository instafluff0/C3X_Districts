...
// One 64 KiB allocation; DISCARD preserves earlier submitted batches. Offsets
...
struct DrawParameterStream {
    ...
    void clear(){if(buffer)buffer->Release();if(context)context->Release();buffer=nullptr;context=nullptr;checked=false;uploads=records=0;}
    ...
    template<class Parameters> bool upload(Parameters const* values,unsigned count){
        ...
        std::memset(mapped.pData,0,count*stride);
        for(unsigned i=0;i<count;++i)std::memcpy(static_cast<char*>(mapped.pData)+i*stride,values+i,sizeof(Parameters));
        context->Unmap(buffer,0);++uploads;records+=count;return true;
    }
    void bind(unsigned slot,unsigned record){
        UINT first=record*(stride/16),count=stride/16;
