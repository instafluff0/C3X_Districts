...
cbuffer TerrainSettings : register(b0)
{
    ...
    float game_sun_intensity;
    float3 game_sun_color;
    float game_shadow_strength;
    float3 game_moon_direction;
    float game_moon_intensity;
    float3 game_moon_color;
    float game_night_activation;
    float3 game_ambient_color;
    float game_environment_exposure;
    float game_water_fresnel;
    float game_water_specular;
    float game_emissive_scale;
    float game_hour;
};
#define lab_mode 8.0
...
#define rivers_enabled 1.0
#define rivers_only 0.0
#define l13_layout 1.0
#define l13a_layout 1.0
#define roads_enabled 1.0
#define roads_only 0.0
#define l14_layout 1.0
#define road_style_override -1.0
#define railroads_enabled 1.0
#define railroads_only 0.0
#define l15_layout 1.0
#define railroad_padding 0.0
#define resources_enabled 1.0
#define resources_only 0.0
#define l16_layout 1.0
#define resource_padding 0.0
#define cities_enabled 1.0
#define cities_only 0.0
#define l17_layout 1.0
#define city_padding 0.0
#define environment_sun_direction game_light_direction
#define environment_sun_intensity game_sun_intensity
#define environment_sun_color game_sun_color
#define environment_shadow_strength game_shadow_strength
#define environment_moon_direction game_moon_direction
#define environment_moon_intensity game_moon_intensity
#define environment_moon_color game_moon_color
#define environment_night_activation game_night_activation
#define environment_ambient_color game_ambient_color
#define environment_exposure game_environment_exposure
#define environment_water_fresnel game_water_fresnel
#define environment_water_specular game_water_specular
#define environment_emissive_scale game_emissive_scale
#define environment_hour game_hour
#else
...
{
    return exposure * (l13a_layout > 0.5 ? environment_exposure : 1.0);
}
...
{
    return l13a_layout > 0.5 ? color / (1.0 + color * 0.30) : color;
}
...
float4 PSFeature(FeaturePixelInput input) : SV_TARGET
{
    ...
    float3 light = l13a_layout > 0.5
        ? frame_illumination(form_normal, 1.0, 1.0)
...
float4 PSMain(PixelInput input) : SV_TARGET
{
    ...
    if (marsh_only > 0.5 && abs(input.real_terrain - 9.0) > 0.25)
        return float4(0.025, 0.028, 0.026, 1.0);
    if (volcano_only > 0.5 && abs(input.real_terrain - 10.0) > 0.25)
        return float4(0.025, 0.028, 0.026, 1.0);
    ...
    if (input.panel < 0.5 && lab_mode > 5.5)
    {
        ...
        return float4(pow(saturate(coast_source * exposure), 1.0 / 2.2), 1.0);
    }
    ...
    if (input.panel > 0.5 && input.surface_kind > 8.5 && input.surface_kind < 9.5)
    {
        ...
        float3 river_light = l13a_layout > 0.5
            ? frame_illumination(river_geometry_normal, 1.0, 1.0)
            : (0.78 + diffuse * 0.36).xxx;
        river *= river_light;
        float river_alpha = lerp(0.30, 0.97, water_weight) * bank_fade;
        return float4(pow(saturate(frame_tone_map(
                          river * frame_output_exposure())), 1.0 / 2.2),
                      river_alpha);
    }
    if (input.panel > 0.5 && input.surface_kind > 5.5 && input.surface_kind < 6.5)
    {
        ...
        if (shoreline_integrated < 0.5)
        {
            ...
            return float4(pow(float3(0.82, 0.87, 0.90), 1.0 / 2.2), legacy_alpha);
        }
        ...
        return float4(pow(foam, 1.0 / 2.2), foam_alpha);
    }
    if (input.panel > 0.5 && input.surface_kind > 4.5 && input.surface_kind < 5.5)
    {
        ...
        if (shoreline_integrated < 0.5)
        {
            ...
            return float4(pow(saturate(legacy_water), 1.0 / 2.2), legacy_alpha);
        }
        ...
        if (l13a_layout > 0.5)
            water *= frame_illumination(water_normal, 1.0, 1.0) * 0.78;
        if (biq_layout > 0.5)
        {
        ...
        if (biq_layout > 0.5)
        {
            ...
            if (l13a_layout > 0.5)
                foam_color *= frame_illumination(float3(0.0, 0.0, 1.0), 1.0, 1.0) * 0.78;
            water = lerp(water, foam_color, foam_alpha);
            alpha = max(alpha, foam_alpha * 0.86);
        }
        return float4(pow(saturate(frame_tone_map(
                          water * frame_output_exposure())), 1.0 / 2.2),
                      alpha);
    }
    if (input.panel > 0.5 && input.surface_kind > 3.5 && input.surface_kind < 4.5)
    {
        ...
        if (shoreline_integrated < 0.5)
        {
            ...
            return float4(pow(saturate(legacy_bed * exposure), 1.0 / 2.2), 1.0);
        }
        ...
        if (l13a_layout > 0.5)
            bed *= frame_illumination(float3(0.0, 0.0, 1.0), 1.0, 1.0) * 0.78;
        return float4(pow(saturate(frame_tone_map(
                          bed * frame_output_exposure())), 1.0 / 2.2),
                      1.0);
    }
    float2 world_position = input.macro_uv * 2.0;
    ...
    if (biq_layout > 0.5)
    {
        ...
        float grass_clutter_weight = weights.x * grass_clutter.a *
            (abs(input.real_terrain - 6.0) < 0.25 ||
             abs(input.real_terrain - 10.0) < 0.25 ? 0.18 : 0.46);
        ...
        if (volcano_enabled > 0.5 && abs(input.real_terrain - 10.0) < 0.25)
        {
            ...
            float active = input.active_effect >= 0.0
                ? input.active_effect : fixture_active;
            ...
            albedo = lerp(albedo, volcano_color,
                          smoothstep(0.02, 0.62, input.authored_relief.y));
        }
    }
    ...
    if (mountain_surface)
    {
        ...
        float3 mountain_base = desert_mountain
            ? desert_mountain_base_texture.Sample(material_sampler, input.uv).rgb
            : mountain_base_texture.Sample(material_sampler, input.uv).rgb;
        float3 mountain_top = desert_mountain
            ? desert_mountain_stripe1_texture.Sample(material_sampler, input.uv).rgb
            : mountain_top_texture.Sample(material_sampler, input.uv).rgb;
        float3 mountain_snow = desert_mountain
            ? desert_mountain_stripe3_texture.Sample(material_sampler, input.uv).rgb
            : mountain_snow_texture.Sample(material_sampler, input.uv).rgb;
        ...
        if (desert_mountain)
        {
            float3 stripe2 = desert_mountain_stripe2_texture.Sample(material_sampler, input.uv).rgb;
    ...
    if (input.panel > 0.5 && lab_mode > 0.5)
    {
        ...
        float3 light = l13a_layout > 0.5
            ? frame_illumination(material_normal, shadow_visibility, ambient_visibility)
    ...
    float3 display_color = pow(saturate(frame_tone_map(
        linear_color * frame_output_exposure())), 1.0 / 2.2);
    return float4(display_color, 1.0);
}
