...
public:
    ...
    bool compose_resource_animations(c3x_renderer_frame_v1 const & frame) {
        ...
        for (auto const & anchor:resource_anchors) {
            ...
            for (std::size_t i=0;i<posed.size();++i) {
                ...
                v={sx,sy,depth,source.uv[0],source.uv[1],
                    ...
                    source.normal[0]*sine+source.normal[1]*cosine,source.normal[2],21.f,
            ...
            for (int y=visible.top/128;y<=(visible.bottom-1)/128;++y)
                for (int x=visible.left/128;x<=(visible.right-1)/128;++x)
        ...
        for(auto const & rect:rectangles) {
            ...
            if(!submit_geometry(buffers,{{0,0,128,128}},settings,block_target,block_depth,128,128,
                    nullptr,true,true,&geometry_vertex_buffers))return false;
            D3D11_BOX box={0,0,0,unsigned(rect.right-rect.left),unsigned(rect.bottom-rect.top),1};
            context->CopySubresourceRegion(render_texture,0,unsigned(rect.left),unsigned(rect.top),0,block_texture,0,&box);
        }
    ...
    bool fill_output(c3x_renderer_frame_v1 const & frame,
        ...
        if (trace.level) {
            ...
            std::snprintf(detail, sizeof(detail),
                ...
                "reused_pixels=%u draw_pixels=%u block_pixels=%u block_bytes=%zu render_ms=%.3f geometry_ms=%.3f draw_submit_ms=%.3f readback_wait_ms=%.3f",
                ...
                trace.milliseconds(frame_draw_ticks), trace.milliseconds(frame_readback_ticks));
    ...
    c3x_renderer::TileFootprint tile_footprint(CachedTileGeometry const & tile,
        ...
        for (auto const & layer : tile.buffers)
            for (auto const & chunk : layer) {
    ...
    void append_tile_geometry(CachedTileGeometry & tile, c3x_renderer_tile_v1 const & record) {
        ...
        for (std::size_t layer = 0; layer < geometry_layer_count; ++layer) {
            ...
            for (CachedVertexChunk chunk : tile.buffers[layer]) {
                ...
                chunk.translation_x = anchor_x - tile.anchor_x;
                chunk.translation_y = anchor_y - tile.anchor_y;
    ...
    bool submit_geometry(std::array<std::vector<CachedVertexChunk>, geometry_layer_count> const & buffers,
        ...
        if (pickup_profile) {
            ...
            if(!source_shadow.prepare(context,shadow_basis,receivers,casters,bind,cancellation)) {
    ...
    bool prepare_pixel_block(std::atomic<bool> const & foreground) {
        ...
        for (auto const & contributor : pending_pixel_block.key) {
            auto found=std::find_if(tile_geometry_cache.begin(),tile_geometry_cache.end(),[&](auto const & item){
                return item.second.version==contributor.mesh;
            });
    ...
    bool render(c3x_renderer_frame_v1 const & frame, c3x_renderer_output_v1 & output,
        ...
        if (reuse_raster) {
            ...
            // Preserve the GPU derivative-quad phase. Native tile steps are
            // even; odd-pixel diagnostic/camera moves reuse meshes but reraster.
            reuse_raster = (raster_dx != 0 || raster_dy != 0) &&
                (raster_dx & 1) == 0 && (raster_dy & 1) == 0 &&
        ...
        for (c3x_renderer_u32 index = 0; index < frame.tile_count; ++index) {
            ...
            if(!prewarming && pickup_profile && (tile.tile_flags&C3X_RENDERER_TILE_RENDER)==0) {
                // The outer full-appearance ring is optional idle work. Keep
                // only the conservative caster ring in this foreground frame.
                int mx=frame.tile_width*2,my=frame.tile_height*2;
            ...
            if (pickup_profile && cliff_assets_ready && coast_detail) {
                ...
                for(auto const& instance:placements) {
                    ...
                    float cosine=float(std::cos(instance.yaw)),sine=float(std::sin(instance.yaw));
                    std::vector<Vertex> transformed(asset.vertices.size());
                    for(std::size_t i=0;i<asset.vertices.size();++i) {
                        auto const& source=asset.vertices[i];auto& v=transformed[i];
                        float wx=float(instance.position.x+(source.position[0]*cosine-source.position[1]*sine)*instance.scale);
                        float wy=float(instance.position.y+(source.position[0]*sine+source.position[1]*cosine)*instance.scale);
                        float wz=float(instance.z+source.position[2]*instance.scale);
                        float local_u=wx-cu,local_v=1-(wy-cr);
                        float base_y=top+(local_u+local_v)*half_h;
                        v.x=left+half_w+(local_u-local_v)*half_w;
                        v.y=base_y-(wz*112-2.5f)*relief_projection_scale;
                        v.z=base_y+(wz*112-2.5f)*relief_projection_scale*.75f+
                            (wz*112-2.5f)*.0012f*float(frame.target_height);
                        v.u=source.uv[0];v.v=source.uv[1];v.panel=1;
                        v.normal_x=source.normal[0]*cosine-source.normal[1]*sine;
                        v.normal_y=-(source.normal[0]*sine+source.normal[1]*cosine);
                        v.normal_z=source.normal[2];v.base_terrain=.48f;
        ...
        if (!raster_rects.empty() && !submit_geometry(geometry_vertex_buffers, raster_rects,
                ...
                c3x_renderer::power_of_two_extent(width), c3x_renderer::power_of_two_extent(height))) return false;
    ...
    bool blit(c3x_renderer_output_v1 const & output, HDC destination) {
...
public:
    ...
    int blit(c3x_renderer_output_v1 const & output, HDC destination) {
        ...
        LARGE_INTEGER started = {}, finished = {};
        QueryPerformanceCounter(&started);
        int result = renderer_state.blit(output, destination)
...
private:
    ...
    void run() {
        ...
        for (;;) {
            ...
            if (!has_job && !stop_requested && renderer_state.pixel_work_pending()) {
                if (wake.wait_for(lock,std::chrono::milliseconds(2),[this]{return has_job || stop_requested;})) continue;
                lock.unlock();
                bool ok=false;
