...
struct SceneTopology : c3x_renderer::render_core::CapturedScene {
    ...
    std::array<std::uint64_t,6> river_source={};
    bool river_source_valid=false;
};
...
public:
    ...
    std::map<std::pair<int,int>,c3x_renderer::render_core::RenderRegionKey> scene_reflection_cells;
    ...
    // Reflected scene cells are world-keyed GPU content, independent of the
    // current screen atlas. Renderer64 can retain a Standard world's water
    // reflections without charging Civ III's address space.
    ...
    void preserve_process_headroom(){
        ...
        // actual free memory before retaining optional GPU pose content.
        bool pressure=memory.ullAvailPhys<(memory_pressured?2048ull:1536ull)*1024*1024;
#else
        bool pressure=memory.ullAvailVirtual<(memory_pressured?1024ull:768ull)*1024*1024;
#endif
        // Idle unit scratch has no readers after submitted draws. D3D retains
        // resources for commands already queued; no flush or fence is needed.
        auto scratch=unit_scene_work.bytes();
#if defined(_WIN64)
        // Keep the reusable 3D body target across ordinary unit turns. The
        // prior one-second release repeatedly reallocated a 75 MiB target in
        // the live trace. Physical pressure still retires it immediately.
        if(scratch && (pressure||now-last_unit_work>=30000))unit_scene_work.reset();
#else
        ...
        if(pressure==memory_pressured && (!pressure || !reflection_regions.gpu_bytes) &&
            limits.regions==render_regions.gpu_limit &&
            limits.units==unit_bodies.gpu_content_limit && scratch==unit_scene_work.bytes())return;
        auto before=render_regions.gpu_bytes+reflection_regions.gpu_bytes+unit_bodies.gpu_content_bytes+scratch;
        memory_pressured=pressure;
        // Retained reflection cells are optional under physical pressure;
        // working attachments and the current atlas remain authoritative.
        render_regions.set_gpu_limit(limits.regions);
        if(pressure)reflection_regions.clear();
        unit_bodies.set_gpu_content_limit(limits.units);
        GlobalMemoryStatusEx(&memory);
        char detail[352];sprintf_s(detail,"active=%u available_virtual=%llu available_physical=%llu released_cache_bytes=%zu reflection_cap=%zu unit_cap=%zu attachments_bytes=%zu composition_publication_bytes=%zu",
            unsigned(pressure),memory.ullAvailVirtual,memory.ullAvailPhys,before-render_regions.gpu_bytes-reflection_regions.gpu_bytes-unit_bodies.gpu_content_bytes-unit_scene_work.bytes(),
    ...
    void memory_sample(char const* phase) {
        ...
        if(now-last_memory_status>=1000){
            ...
            if(GlobalMemoryStatusEx(&memory)){
                char status[192];sprintf_s(status,"phase=%s available_virtual=%llu available_physical=%llu available_pagefile=%llu",
                    phase,memory.ullAvailVirtual,memory.ullAvailPhys,memory.ullAvailPageFile);
                trace.write("process-memory",status,true);
            }
#ifdef C3X_HELPER_TRIAL
            if(device){
                   ...
                   SUCCEEDED(dxgi->GetAdapter(&base)) && SUCCEEDED(base.As(&adapter))){
                    ...
                    if(SUCCEEDED(adapter->QueryVideoMemoryInfo(0,DXGI_MEMORY_SEGMENT_GROUP_LOCAL,&local)) &&
                       SUCCEEDED(adapter->QueryVideoMemoryInfo(0,DXGI_MEMORY_SEGMENT_GROUP_NON_LOCAL,&nonlocal))){
                        char status[256];sprintf_s(status,
                            "local_budget=%llu local_usage=%llu nonlocal_budget=%llu nonlocal_usage=%llu",
    ...
    unsigned frame_tile_missing_instance=0,frame_tile_missing_binding=0,
        frame_tile_context_mismatch=0,frame_tile_dependency_mismatch=0;
    unsigned frame_tile_invalid_shared=0,frame_tile_invalid_appearance=0,
        frame_tile_invalid_semantic=0,frame_tile_invalid_coast=0,
        frame_tile_invalid_world=0,frame_tile_invalid_anchor=0,frame_tile_invalid_river=0;
    ...
    void trim_to_prepared(c3x_renderer_benchmark_oracle_trim_v1 & result, bool oracle_limits=true) {
        ...
        result.cleared_region_bytes = render_regions.gpu_bytes + render_regions.metadata_bytes +
            reflection_regions.gpu_bytes + reflection_regions.metadata_bytes;
        ...
        render_regions.clear();reflection_regions.clear();
    ...
    static bool read_file(char const * path, std::vector<std::uint8_t> & output) {
        ...
        if (!ok)
            output.clear();
        c3x_inputs::runtime().asset(path,output.data(),output.size(),ok);
        return ok;
    }
    ...
    static c3x_renderer_i64 resource_clock(c3x_renderer_frame_v1 const & frame,int samples_per_second=15) {
        return frame.presentation_time_ticks/std::max<c3x_renderer_i64>(1,frame.presentation_frequency/samples_per_second);
    }
    ...
    bool submit_scene_pass(GeometryDrawView inputs,std::vector<unsigned> const& order,bool dynamic_pass,
        ...
        using Shadow=c3x_renderer::render_core::SourceShadow;
        auto& retained=material_pass?material_submission:static_submission;
        bool retain=inputs.is(geometry_vertex_buffers) && (material_pass || !dynamic_pass);
        ...
        if(reused){
            ...
            if(dynamic_pass)selected_dynamic+=retained.selected;else selected_static+=retained.selected;
            ...
            char detail[160];sprintf_s(detail,"reused=1 batches=%zu selected=%u bytes=%zu caster_builds=0 selection_scans=0",
                retained.batches.size(),retained.selected,retained.bytes);trace.write(material_pass?"material-submission":"static-submission",detail,true);
            return true;
        }
        if(retain)retained={};
        std::vector<Shadow::Caster> temporary_casters;Shadow::PreparedCasters prepared;
        bool retained_casters=dynamic_pass && !material_pass && material_submission.signature && material_submission.signature==cached_signature.complete;
        auto& casters=retained_casters?material_submission.casters:temporary_casters;
        auto prepared_ptr=retained_casters?&material_submission.prepared:prepare_shadow_submission(geometry_vertex_buffers,casters,prepared);
        auto spans=c3x_renderer::render_core::scene_spans<D3D11_RECT>(int(w),int(h),region_origin_x,region_origin_y);
            ...
            bool indexed=retained_world && inputs.is(geometry_vertex_buffers) && region_contributors.ready;
            if(indexed){
            ...
            auto flush=[&](){
                ...
                if(retain && ok){
                    ...
                    retained.bytes+=sizeof(SceneSubmission::Batch)+count*sizeof(GeometryDrawRecord)+rectangles.size()*sizeof(D3D11_RECT)+pages.size()*64;
    ...
    bool prepare_scene_reflection() {
        if(!reflection.enabled){
            scene_reflection_cells.clear();scene_reflection_signature=0;
            release(scene_reflection_clear);release(scene_reflection_view);release(scene_reflection_texture);
            scene_reflection_width=scene_reflection_height=0;reflection.linear.reset();return true;
        }
        ...
        unsigned built=0,reused=0,unneeded=0,reflection_hits=0,reflection_stores=0;
        std::size_t metadata_bytes=0;
        auto draw_calls_before=frame_draw_calls;
        LARGE_INTEGER begin={},end={};QueryPerformanceCounter(&begin);
        if(scene_reflection_signature!=cached_signature.complete){
            // Atlas contents have one owner. Keys describe those exact cells,
            // not a second texture cache. A failed partial update invalidates
            // all keys before any retry can read overwritten samples.
            auto previous_cells=std::move(scene_reflection_cells);
            scene_reflection_cells.clear();scene_reflection_signature=0;
            using Shadow=c3x_renderer::render_core::SourceShadow;
            ...
            std::map<std::pair<int,int>,c3x_renderer::render_core::RenderRegionKey> next_cells;
            ...
            for(int y=first_y;y<int(h);y+=128)for(int x=first_x;x<int(w);x+=128){
                ...
                if(needed && visibility_pass)
                    needed=visibility_coverage.may_contribute(x-24,y-24,x+144,y+144);
                ...
                // This entry stores only the mirrored scene. The ordinary
                // land-pass proof belongs to the separate terrain cache.
                bool cacheable=!world_regions_control && render_region_key(geometry_vertex_buffers,local,casters,prepared_ptr,key,nullptr,true);
                if(cacheable)key.push_back(0x7265666c65637431ull); // resolved RGBA16F mirror, not finished BGRA
                auto cell=std::make_pair(x,y);auto old=previous_cells.find(cell);
                bool same=cacheable && old!=previous_cells.end() && old->second==key;
                auto bytes=key.capacity()*sizeof(std::uint64_t)+96;
                c3x_renderer::render_core::RenderRegionKey const* cache_key=nullptr;
                if(cacheable && metadata_bytes+bytes<=16u*1024u*1024u){
                    metadata_bytes+=bytes;
                    cache_key=&next_cells.emplace(cell,std::move(key)).first->second;
                }
                ...
                if(cache_key && reflection_regions.gpu_limit){
                    if(auto cached=reflection_regions.find(*cache_key)){
                        context->OMSetRenderTargets(0,nullptr,nullptr);
                        context->CopySubresourceRegion(scene_reflection_texture,0,UINT(l*2),UINT(t*2),0,cached,0,&box);
                        ++reused;++reflection_hits;continue;
                    }
                }
                ...
                context->CopySubresourceRegion(scene_reflection_texture,0,UINT(l*2),UINT(t*2),0,
                    reflection.linear.resolved,0,&box);
                // Store exactly the source pixels used above. A later camera
                // can populate its atlas by GPU copy without replaying the
                // reflected geometry and shadow pass for this world cell.
                constexpr std::size_t cell_bytes=256u*256u*8u;
                if(cache_key && reflection_regions.make_room(*cache_key,cell_bytes)){
                    D3D11_TEXTURE2D_DESC description={};description.Width=description.Height=256;
                    description.MipLevels=description.ArraySize=description.SampleDesc.Count=1;
                    description.Format=DXGI_FORMAT_R16G16B16A16_FLOAT;
                    ID3D11Texture2D* image=nullptr;
                    if(SUCCEEDED(device->CreateTexture2D(&description,nullptr,&image))){
                        D3D11_BOX source={0,0,0,256,256,1};
                        context->CopySubresourceRegion(image,0,0,0,0,reflection.linear.resolved,0,&source);
                        if(reflection_regions.insert(*cache_key,image,cell_bytes)){image=nullptr;++reflection_stores;}
                    }
                    if(image)image->Release();
                }
            }
            scene_reflection_cells=std::move(next_cells);
            scene_reflection_signature=cached_signature.complete;
        }
        QueryPerformanceCounter(&end);
        char detail[416];sprintf_s(detail,"built=%u reused=%u unneeded=%u cache_hits=%u cache_stores=%u draws=%llu atlas_bytes=%zu cache_bytes=%zu cache_metadata=%zu ms=%.3f",
            built,reused,unneeded,reflection_hits,reflection_stores,static_cast<unsigned long long>(frame_draw_calls-draw_calls_before),
            std::size_t(w)*h*32,reflection_regions.gpu_bytes,reflection_regions.metadata_bytes,
            trace.milliseconds(end.QuadPart-begin.QuadPart));
    ...
    bool compose_scene_surface(GeometryDrawView dynamic) {
        ...
        // Retain only visible samples plus the finishing filter margin. The
        // retired background guard consumed hundreds of MiB without making
        // arbitrary destinations resident; camera damage already redraws exposed spans.
        // One scene-linear sample set; static damage redraws use retained geometry.
        auto& glow=region_glow;
        unsigned w=unsigned(width)+8,h=unsigned(height)+8;
        using Budget=c3x_renderer::render_core::FrameWorkingSet;
        std::size_t target_bytes=Budget::scene(unsigned(width),unsigned(height));
        if(!city_profile || !c3x_renderer::render_core::scene_surface_extent(width,height) ||
           target_bytes>Budget::scene_limit) {
            trace.write("scene-surface-failed","bounded scene view/target contract",true);return false;
        }
        // Reserve the final working set before allocating replacements. Do
        // not let old cache capacity compete with required frame attachments.
        ...
        for(auto layer:layers)for(auto item:inputs[layer]){
            if(inputs.is(geometry_vertex_buffers) && (!item.water_dependent() || !item.water_visible()))continue;
        ...
        auto redraw=scene_dynamic_damage;
        redraw.insert(redraw.end(),dynamic_damage.begin(),dynamic_damage.end());
        auto exposed=physical(static_rectangles);
        if(!restored)redraw.insert(redraw.end(),exposed.begin(),exposed.end());
        redraw=disjoint(redraw);
        if(restored || translated){
            if(!redraw.empty() && !scene_restore.draw(context,linear,nullptr,nullptr,0,0,{},&redraw,0,0,false,true))return false;
        }else{
        ...
        std::vector<c3x_renderer::city_fidelity::Lighting const*> scene_lights;
        for(auto const& item:geometry_vertex_buffers[geometry_city])if(item.content().city_lighting){
            ...
            if(std::find(scene_lights.begin(),scene_lights.end(),pointer)==scene_lights.end())scene_lights.push_back(pointer);
        }
        if(!cities.lights(context,scene_lights))return false;
        if(!redraw.empty() && !submit(geometry_vertex_buffers,static_scene_order(),false))return false;
        context->OMSetRenderTargets(0,nullptr,nullptr);
        ...
        if(water_scene_active){
            if(!submit(geometry_vertex_buffers,water_scene_order(),true,true))return false;
        }
        ...
        auto& output=pixels;if(!gpu_output_mode)output.resize(std::size_t(width)*height);
        ...
        target_bytes=std::size_t(w)*h*240u;
        char detail[768];sprintf_s(detail,"static_reused=%u translated=%u damage_rects=%zu static_selected=%u dynamic_selected=%u batches=%u target_bytes=%zu target_cap=%zu resolves=%u readbacks=%u full_surface_copies=0 backup_reused=%u backup_bytes=%zu dynamic_damage_rects=%zu copied_rects=%zu static_submit_ms=%.3f dynamic_submit_ms=%.3f finish_submit_ms=%.3f completion_wait_ms=%.3f cpu_copy_ms=%.3f",
            unsigned(restored),unsigned(translated),static_rectangles.size(),selected_static,selected_dynamic,batches,target_bytes,std::size_t(672u)*1024u*1024u,unsigned(!finish_damage.empty()),unsigned(!gpu_output_mode && !copies.empty()),0u,std::size_t(0),scene_dynamic_damage.size(),copies.size(),
        ...
        sprintf_s(detail,"attachments=%zu composition_publication_bytes=%zu cache_bytes=%zu cache_capacity=%zu logical_limit=%zu process_va_separate=1",
            ...
            render_regions.gpu_limit+unit_bodies.gpu_content_limit,Budget::cache_limit);
    ...
    bool fill_output(c3x_renderer_frame_v1 const & frame,
        ...
        if(trace.level>=2){
            char residency_detail[192];std::snprintf(residency_detail,sizeof(residency_detail),
                "missing_instance=%u missing_binding=%u context_mismatch=%u dependency_mismatch=%u built=%u reused=%u",
                frame_tile_missing_instance,frame_tile_missing_binding,frame_tile_context_mismatch,
                frame_tile_dependency_mismatch,frame_tiles_built,frame_tiles_reused);
            trace.write("tile-residency-reasons",residency_detail,true);
            std::snprintf(residency_detail,sizeof(residency_detail),
                "shared=%u appearance=%u semantic=%u coast=%u world=%u anchor=%u river=%u",
                frame_tile_invalid_shared,frame_tile_invalid_appearance,frame_tile_invalid_semantic,
                frame_tile_invalid_coast,frame_tile_invalid_world,frame_tile_invalid_anchor,frame_tile_invalid_river);
            trace.write("tile-dependency-reasons",residency_detail,true);
        }
    ...
    bool cache_geometry_layer(c3x_renderer::render_core::ImmutableMeshUpload& upload,
        ...
        // Renderer64's world residency is governed by the main geometry
        // budget. The old 64 MiB prefetch pool was a second, smaller ceiling
    ...
    bool tile_content_valid(CachedTileGeometry& cached,c3x_renderer_tile_v1 const& tile) {
        ...
        for (auto const & dependency : cached.coast_dependencies)
            if (world_coast.node_revision(dependency.first) != dependency.second) { valid = false; ++frame_tile_invalid_coast; break; }
        for (auto const & dependency : cached.world_dependencies)
            if (world_coast.world().at(dependency.first) != dependency.second) { valid = false; ++frame_tile_invalid_world; break; }
        for (auto const & dependency : cached.anchor_dependencies) {
            ...
            if (current == nullptr ||
                ...
                std::int64_t(current->occurrence.anchor_y-tile.anchor_y)*(cached.world_ground?cached.source_tile_width:1) !=
                    std::int64_t(dependency.second[1])*(cached.world_ground?shadow_tile_width:1)) {valid = false;++frame_tile_invalid_anchor;break;}
        }
        bool river_valid=natural.valid(cached.river_dependencies);
    ...
    bool restore_viewport_geometry(CachedViewport const& stored,c3x_renderer_frame_v1 const& frame,
        ...
        geometry_cache.signature=signature;geometry_cache.tiles=stored.tiles;geometry_cache.selection=stored.selection;
    ...
    bool render_region_key(GeometryDrawView buffers,
            ...
            c3x_renderer::render_core::SourceShadow::PreparedCasters* prepared,
            c3x_renderer::render_core::RenderRegionKey& key,std::vector<std::size_t>* sections=nullptr,bool reflection_only=false) {
        using Shadow=c3x_renderer::render_core::SourceShadow;
        ...
        for(unsigned pass=reflection_only?1u:0u;pass<(reflection.enabled?2u:1u);++pass){
    ...
    bool geometry_matches(CachedGeometry const & candidate,
                          ...
                          c3x_renderer::TerrainFrameSignature const & signature,
                          c3x_renderer::render_core::ForegroundSelection const& selection,
                          int & translation_x, int & translation_y,
                          bool require_translation) const {
        if (!candidate.valid || !(candidate.selection==selection) || signature.geometry != candidate.signature.geometry ||
    ...
    bool reuse_geometry_for_translation(
        ...
        return geometry_matches(geometry_cache, frame, signature,selection,
    ...
    bool render(c3x_renderer_frame_v1 const & frame, c3x_renderer_output_v1 & output,
        ...
        GetEnvironmentVariableA("C3X_RENDERER_PREFETCH_FOREGROUND_CONTROL",prefetch_foreground_control,sizeof(prefetch_foreground_control));
        ...
        if(city_profile && !water_motion && std::memcmp(water_material.camera,next_water_material.camera,sizeof(water_material.camera))){
            ...
            clear_resource_backdrops();scene_static_signature=0;
        }
        ...
        frame_tile_invalid_shared=frame_tile_invalid_appearance=frame_tile_invalid_semantic=frame_tile_invalid_coast=
            frame_tile_invalid_world=frame_tile_invalid_anchor=frame_tile_invalid_river=0;
        ...
        bool const world_river_source=world_ground && frame.world_topology &&
            frame.world_width_tiles>0 && frame.world_height_tiles>0 &&
            frame.world_topology_count==std::size_t(frame.world_width_tiles)*frame.world_height_tiles/2;
        std::array<std::uint64_t,6> river_source={
            topology_cache.scope_sequence(),std::uint64_t(frame.world_topology_revision),
            std::uint64_t(frame.world_width_tiles),std::uint64_t(frame.world_height_tiles),
            std::uint64_t(frame.world_wrap_x),std::uint64_t(frame.world_wrap_y)};
        bool const rebuild_rivers=!world_river_source || !topology_cache.river_source_valid ||
            topology_cache.river_source!=river_source;
        if(rebuild_rivers)river_nodes.clear();
        for (c3x_renderer_u32 index = 0; index < frame.tile_count; ++index) {
        ...
        if(rebuild_rivers){
            ...
            auto visit_river_tiles=[&](auto visit){
                if(world_river_source){
                    unsigned half=unsigned(frame.world_width_tiles/2);
                    int wx=frame.world_wrap_x?1:0,wy=frame.world_wrap_y?1:0;
                    for(unsigned index=0;index<frame.world_topology_count;++index){
                        if((index&63u)==0 && cancelled())return false;
                        unsigned bits=frame.world_topology[index];
                        int y=int(index/half),x=int((index%half)*2)+(y&1);
                        for(int cy=-wy;cy<=wy;++cy)for(int cx=-wx;cx<=wx;++cx){
                            c3x_renderer_tile_v1 tile={};
                            tile.tile_x=x+cx*frame.world_width_tiles;
                            tile.tile_y=y+cy*frame.world_height_tiles;
                            tile.terrain_type=int(bits&255u);
                            tile.real_terrain_type=int((bits>>8)&255u);
                            tile.river_code=(bits>>16)&255u;
                            visit(tile);
                        }
                    }
                }else for(c3x_renderer_u32 index=0;index<frame.tile_count;++index){
                    if((index&63u)==0 && cancelled())return false;
                    auto const& tile=frame.tiles[index];
                    if(tile.tile_flags&(C3X_RENDERER_TILE_RENDER|C3X_RENDERER_TILE_TOPOLOGY_HALO))visit(tile);
                }
                return true;
            };
            if(!visit_river_tiles([&](auto const& tile){
                unsigned mask=tile.river_code&170u;
                if(mask&2u)add_river_edge(tile.tile_x,tile.tile_y-1,tile.tile_x+1,tile.tile_y);
                if(mask&8u)add_river_edge(tile.tile_x+1,tile.tile_y,tile.tile_x,tile.tile_y+1);
            }))return false;
            constexpr int river_corner_offsets[4][2]={{0,-1},{1,0},{0,1},{-1,0}};
            if(!visit_river_tiles([&](auto const& tile){
                if(ground_type(tile)<11)return;
            ...
            std::sort(river_nodes.begin(),river_nodes.end(),[](auto const& a,auto const& b){
                return a.lattice_x<b.lattice_x || (a.lattice_x==b.lattice_x && a.lattice_y<b.lattice_y);
            });
            topology_cache.river_source_valid=world_river_source;
            if(world_river_source)topology_cache.river_source=river_source;
        }
        ...
        auto select_river_nodes=[&](c3x_renderer_tile_v1 const& tile){
            ...
            if (river_assets_ready && (tile.river_code & 170u) != 0) {
                ...
                int x_limit=int(std::ceil(1.0f+node_window.radius/node_window.half_width));
                auto node=std::lower_bound(river_nodes.begin(),river_nodes.end(),tile.tile_x-x_limit,
                    [](RiverNode const& value,int x){return value.lattice_x<x;});
                for(;node!=river_nodes.end() && node->lattice_x<=tile.tile_x+x_limit;++node)
        ...
        for (c3x_renderer_u32 preparation_slot = 0; preparation_slot < (batch_preparing?preparation_count:frame.tile_count+compiling_tiles.size()); ++preparation_slot) {
            ...
            if(retained_world && (!prewarming || world_preparation) && persistent_instance){
                ...
                for(auto handle:persistent_instance->compiled_views){
                    ...
                    had_binding=true;
                    if(candidate->compile_context!=compile_context)continue;
                    context_match=true;
                    if(tile_content_valid(*candidate,tile)){ready=candidate;break;}
                }
                ...
                if(!had_binding)++frame_tile_missing_binding;
                else if(!context_match)++frame_tile_context_mismatch;
                else ++frame_tile_dependency_mismatch;
            }
            else if(retained_world && (!prewarming || world_preparation))++frame_tile_missing_instance;
...
struct PublishedMapFrame {
    ...
    bool refresh_view(c3x_renderer_frame_v1 const& current) {
        auto normalized=current;normalized.dirty_flags=frame.dirty_flags;
        if(current.presentation_time_ticks<frame.presentation_time_ticks || !matches_static_view(normalized))return false;
        auto sampled_ticks=frame.presentation_time_ticks;
        unsigned ambient=output.visible_animation_count>frame.visible_animation_count?
            output.visible_animation_count-frame.visible_animation_count:0;
        frame=current;frame.tiles=occurrences.empty()?nullptr:occurrences.data();
        frame.world_topology=nullptr;frame.world_topology_count=0;frame.presentation_time_ticks=sampled_ticks;
        ...
        output.visible_animation_count=current.visible_animation_count+ambient;
        output.request_continuous_redraw=output.visible_animation_count!=0;
        return true;
    }
    void clear() {
...
public:
    ...
    int render_gpu(c3x_renderer_camera_request_v1 const& request,c3x_renderer_gpu_frame_v1& view,c3x_renderer_output_v1& metadata){
        ...
        result=adopt_gpu_camera_locked(lock,ticket,view,metadata);
        QueryPerformanceCounter(&end);
    ...
    int trial_export_shared(c3x_renderer_i64 ticket,c3x_renderer_i64 image,DWORD consumer_pid,
        ...
        std::lock_guard<std::mutex> calls(call_mutex);std::unique_lock<std::mutex> lock(state_mutex);
        trial_ticket=ticket;trial_image=image;trial_consumer_pid=consumer_pid;trial_raw=raw;
    ...
    int poll_gpu_camera_view(c3x_renderer_i64 ticket,c3x_renderer_gpu_camera_view_v1& view){
        ...
        if(result==C3X_RENDERER_RESULT_OK){
            ...
            next.camera.ticket=ticket;next.camera.identity=gpu_publication.identity;next.camera.frame=gpu_publication.frame;
            next.pixel_phase_x=gpu_publication.phase_x;next.pixel_phase_y=gpu_publication.phase_y;
            view=next;
        }
        return result;
    }
private:
    int begin_gpu_camera_locked(c3x_renderer_camera_request_v1 const& request,c3x_renderer_i64& ticket,bool fresh=false){
        ...
        if(camera_gpu && camera_ticket>0 && (camera_result==C3X_RENDERER_RESULT_PENDING || camera_result==C3X_RENDERER_RESULT_OK)){
            bool same=camera_pending?same_camera_request(frame,request.identity,camera_pending_frame,camera_pending_identity):
                (job_camera_ticket==camera_ticket && !camera_cancelled.load(std::memory_order_relaxed) &&
                 same_camera_request(frame,request.identity,job_frame,job_camera_identity));
            if(same && (!fresh || gpu_camera_front_ticket!=camera_ticket)){ticket=camera_ticket;return C3X_RENDERER_RESULT_PENDING;}
        }
        if(!publish_scene_capture(frame,request.identity))return C3X_RENDERER_RESULT_ERROR;
...
public:
    ...
    void capture_world_tick(UINT_PTR id){
        ...
        c3x_inputs::Call input(c3x_inputs::Kind::world_page,0,[&](auto& out){
            out(page.first);out(page.capacity);out(page.count);c3x_inputs::c3x_renderer_camera_identity_v1_fields(out,page.identity);
            c3x_inputs::frame(out,page.frame);out(std::int32_t(result));
            c3x_inputs::require(page.count<=page.capacity&&page.count<=128,"input world page limit");
    ...
    int replay_world_page(c3x_renderer_world_page_v1 const& recorded,int callback_result){
        std::lock_guard<std::mutex> calls(call_mutex);std::unique_lock<std::mutex> lock(state_mutex);
        auto state=scene_changes.state();if(!state)return 0;
        auto page=world_input.page(*state);
        if(recorded.first!=page.first||recorded.capacity!=page.capacity||recorded.count>page.capacity)return 0;
        page.identity=recorded.identity;page.frame=recorded.frame;page.count=recorded.count;
        for(unsigned n=0;n<page.count;++n)page.tiles[n]=recorded.tiles[n];
    ...
    int visual_policy(unsigned policy){
        ...
        int ready=session&&session->visual_ready()?1:0;
        if(policy==2&&ready!=last_visual_ready){
            char detail[224];std::snprintf(detail,sizeof(detail),
                "ready=%d active=%d nodes=%zu sources=%zu bytes=%llu map_samples=%llu",
                ready,session&&session->visual_active()?1:0,session?session->visual_nodes():0,
                session?session->visual_sources():0,
                static_cast<unsigned long long>(session?session->visual_bytes():0),
                static_cast<unsigned long long>(visual_map_samples));
            renderer_state.trace.write("visual-readiness",detail,true);last_visual_ready=ready;
        }
        return input.result(policy==3?(session&&session->visual_active()?1:0):ready);
    }
    ...
    int present_gpu(c3x_renderer_gpu_present_v1 const& request){
        ...
        try{
            ...
            if(result!=C3X_RENDERER_RESULT_OK){
                ...
                char manual[4]={};
                if(GetEnvironmentVariableA("C3X_RENDERER_MANUAL_VISUAL",manual,sizeof(manual))!=1||manual[0]!='1')
    ...
    int native_screen(c3x_native_images::ScreenSnapshot* screen) {
        std::lock_guard<std::mutex> calls(call_mutex);
        c3x_inputs::Call input(c3x_inputs::Kind::native_snapshot,0,[&](auto& out){out.u32(screen?1:0);if(!screen){input_screen={};return;}
            out(std::int32_t(screen->width));out(std::int32_t(screen->height));out(screen->native_format);
            out(std::int32_t(screen->area.left));out(std::int32_t(screen->area.top));out(std::int32_t(screen->area.right));out(std::int32_t(screen->area.bottom));
            input_screen.encode(out,screen->pixels);});
        stop_visual_delivery();
        ...
        if(!screen){
            ...
            gpu_presenter.release_native();native_screen_active=false;return input.result(C3X_RENDERER_RESULT_OK);
        }
        start_locked();
        ...
        screen_upload=false;
        if(submit_locked(lock,Command::native_screen)!=C3X_RENDERER_RESULT_OK || !renderer_state.device)return input.result(C3X_RENDERER_RESULT_BAD_ARGUMENT);
        try {
            ...
            if(!gpu_presenter.prepare(screen->window,renderer_state.device,screen->width,screen->height,full)){
                gpu_presenter.release_native();native_screen_active=false;return input.result(C3X_RENDERER_RESULT_BAD_ARGUMENT);
            }
            ...
            return input.result(result);
        }catch(...){gpu_presenter.release_native();native_screen_active=false;return input.result(C3X_RENDERER_RESULT_ERROR);}
    }
    ...
    int render(c3x_renderer_frame_v1 const & frame, c3x_renderer_output_v1 & output,
        ...
        bool adopt=!camera_gpu && camera_ticket>0 && !camera_paused &&
...
private:
    ...
    int enqueue_camera_locked(c3x_renderer_frame_v1 const& frame,
                              c3x_renderer_camera_identity_v1 const& identity,
                              c3x_renderer_i64& ticket,bool gpu=false) {
        if(renderer_state.camera_serial==INT64_MAX)return C3X_RENDERER_RESULT_ERROR;
        ...
        if(gpu || !compatible_ahead(frame,identity)) {
...
public:
    ...
    int camera_present_view(c3x_renderer_camera_request_v1 const& request,
        ...
        if(!publication.refresh_view(*request.frame))return C3X_RENDERER_RESULT_PENDING;
...
private:
    ...
    c3x_inputs::ScreenInput input_screen;
    bool input_display_readback=false;unsigned input_display_width=0,input_display_height=0;
    ...
    c3x_gpu_images::RetainedComposition::Direct unit_scene_operation(c3x_gpu_images::Rect original_envelope){
        ...
        operation.draw=[this,state,original_envelope](c3x_gpu_images::Compositor& target,c3x_gpu_images::Command const& command){
        ...
        for (;;) {
            ...
            if(command==Command::native_screen){
                ...
                if(source&&trial_consumer_pid){
                    ...
                    desc.Format=trial_raw?DXGI_FORMAT_R32_UINT:DXGI_FORMAT_B8G8R8A8_UNORM;
                    desc.BindFlags=trial_raw?D3D11_BIND_SHADER_RESOURCE:
                    ...
                    if(SUCCEEDED(hr)&&!trial_raw)hr=renderer_state.device->CreateRenderTargetView(shared.Get(),nullptr,&target);
                ...
                if(trial_surface_handle&&trial_surface_width>0&&trial_surface_height>0&&
                   trial_surface_width<=2240&&trial_surface_height<=1260&&renderer_state.initialize_device()){
                    ...
                    if(SUCCEEDED(hr)){
                        D3D11_TEXTURE2D_DESC scratch={};trial_surface_back->GetDesc(&scratch);
                ...
                if(p.action==1||p.action==2){
                    ...
                    auto* session=renderer_state.gpu_composition.get();
                    result=C3X_RENDERER_RESULT_SUPERSEDED;
                    if(trial_surface_swap&&trial_surface_back&&trial_surface_view&&trial_surface_buffer&&session&&
                       ...
                       p.width<=2240&&p.height<=1260&&p.image>0){
                        ...
                        if(!trial_display){
                            if(!full)result=C3X_RENDERER_RESULT_BAD_ARGUMENT;
                                ...
                                D3D11_TEXTURE2D_DESC desc={};desc.Width=unsigned(p.width);desc.Height=unsigned(p.height);
                                desc.MipLevels=desc.ArraySize=desc.SampleDesc.Count=1;
                                desc.Format=DXGI_FORMAT_B8G8R8A8_UNORM;
                                desc.BindFlags=D3D11_BIND_RENDER_TARGET|D3D11_BIND_SHADER_RESOURCE;
                                desc.MiscFlags=D3D11_RESOURCE_MISC_SHARED_NTHANDLE|D3D11_RESOURCE_MISC_SHARED_KEYEDMUTEX;
                                HRESULT hr=renderer_state.device->CreateTexture2D(&desc,nullptr,&trial_display);
                                if(SUCCEEDED(hr))hr=renderer_state.device->CreateRenderTargetView(trial_display.Get(),nullptr,&trial_display_view);
                ...
                if(trial_display&&trial_display_view&&trial_display_mutex&&trial_buffer&&
                   renderer_state.gpu_composition&&visual_frequency>0&&trial_consumer_pid){
                    ...
                    if(hr!=S_OK)result=C3X_RENDERER_RESULT_DEVICE_ERROR;
                        ...
                        // A renderer no-op retains the previous completed
                        // frame; the x86 presenter can still display it.
                        hr=trial_display_mutex->ReleaseSync(drawn>=1?1:0);renderer_state.context->Flush();
                        result=FAILED(hr)?C3X_RENDERER_RESULT_DEVICE_ERROR:
                            drawn>=1?C3X_RENDERER_RESULT_OK:C3X_RENDERER_RESULT_PENDING;
                        if(result==C3X_RENDERER_RESULT_OK){
                            D3D11_TEXTURE2D_DESC desc={};trial_display->GetDesc(&desc);
                ...
                if(renderer_state.gpu_composition&&renderer_state.gpu_composition->current_ticket()==gpu_unit.ticket){
                    ...
                    if(body.direct_scene){
                        ...
                        c3x_gpu_images::Rect envelope={job_unit.body_x-margin_x,job_unit.body_y-margin_y,
                            ...
                            job_unit.body_y-margin_y+body_height+2*margin_y};
                        result=renderer_state.gpu_composition->draw_dynamic(gpu_unit,
                            unsigned(body_width+2*margin_x),unsigned(body_height+2*margin_y),
                            envelope.left,envelope.top,unit_scene_operation(envelope));
...
extern "C" __declspec(dllexport) int c3x_renderer_camera_present_view(
    ...
    c3x_inputs::Call input(c3x_inputs::Kind::camera,5,[&](auto& out){c3x_inputs::c3x_renderer_camera_identity_v1_fields(out,request->identity);c3x_inputs::frame(out,*request->frame);});
    int result=get_renderer_worker().camera_present_view(*request,*view);return input.result(result,[&](auto& out){c3x_inputs::adoption_witness(out,*view,result);});
}
...
extern "C" __declspec(dllexport) int c3x_renderer_camera_begin(
    ...
    c3x_inputs::Call input(c3x_inputs::Kind::camera,8,[&](auto& out){c3x_inputs::frame(out,*frame);});
    int code=get_renderer_worker().camera_begin(*frame,*ticket);
    return input.result(code,[&](auto& out){out(std::int64_t(code==C3X_RENDERER_RESULT_PENDING?*ticket:0));});
}
...
extern "C" __declspec(dllexport) int c3x_renderer_camera_poll(
    ...
    if(!output || output->api_version!=C3X_RENDERER_API_VERSION || output->struct_size!=sizeof(*output))
        return C3X_RENDERER_RESULT_BAD_ARGUMENT;
    c3x_inputs::Call input(c3x_inputs::Kind::camera,10,[&](auto& out){out(ticket);});
...
extern "C" __declspec(dllexport) int c3x_renderer_camera_begin_view(
    ...
    c3x_inputs::Call input(c3x_inputs::Kind::camera,9,[&](auto& out){c3x_inputs::c3x_renderer_camera_identity_v1_fields(out,request->identity);c3x_inputs::frame(out,*request->frame);});
    int code=get_renderer_worker().camera_begin(*request->frame,*ticket,request->identity);
    return input.result(code,[&](auto& out){out(std::int64_t(code==C3X_RENDERER_RESULT_PENDING?*ticket:0));});
}
...
extern "C" __declspec(dllexport) int c3x_renderer_prepare_view(
    ...
    if(!request || request->version!=C3X_RENDERER_CAMERA_VIEW_VERSION || request->struct_size!=sizeof(*request) ||
       !valid_frame(request->frame,&output))return C3X_RENDERER_RESULT_BAD_ARGUMENT;
    c3x_inputs::Call input(c3x_inputs::Kind::camera,6,[&](auto& out){out(std::int32_t(query_only));c3x_inputs::c3x_renderer_camera_identity_v1_fields(out,request->identity);c3x_inputs::frame(out,*request->frame);});
    try{return input.result(get_renderer_worker().prepare_view(*request,query_only!=0));}catch(...){return input.result(C3X_RENDERER_RESULT_ERROR);}
}
...
extern "C" __declspec(dllexport) int c3x_renderer_prepare_nearby_view(
    ...
    if(!request || request->version!=C3X_RENDERER_CAMERA_VIEW_VERSION || request->struct_size!=sizeof(*request) ||
       !valid_frame(request->frame,&output))return C3X_RENDERER_RESULT_BAD_ARGUMENT;
    c3x_inputs::Call input(c3x_inputs::Kind::camera,7,[&](auto& out){c3x_inputs::c3x_renderer_camera_identity_v1_fields(out,request->identity);c3x_inputs::frame(out,*request->frame);});
    try{return input.result(get_renderer_worker().prepare_nearby_view(*request));}
    catch(...) {return input.result(C3X_RENDERER_RESULT_ERROR);}
}
...
extern "C" __declspec(dllexport) int c3x_renderer_camera_poll_view(
    ...
    if(!view || view->version!=C3X_RENDERER_CAMERA_VIEW_VERSION || view->struct_size!=sizeof(*view))
        return C3X_RENDERER_RESULT_BAD_ARGUMENT;
    c3x_inputs::Call input(c3x_inputs::Kind::camera,11,[&](auto& out){out(ticket);});
    int code=renderer_worker?renderer_worker->camera_poll_view(ticket,*view):C3X_RENDERER_RESULT_SUPERSEDED;
    return input.result(code,[&](auto& out){c3x_inputs::adoption_witness(out,*view,code);});
}
...
extern "C" __declspec(dllexport) int c3x_renderer_trial_export_shared_raw(
    c3x_renderer_i64 ticket,c3x_renderer_i64 image,DWORD consumer_pid,
    std::uint64_t* handle,unsigned* width,unsigned* height){
    if(!handle||!width||!height)return C3X_RENDERER_RESULT_BAD_ARGUMENT;
    return get_renderer_worker().trial_export_shared(ticket,image,consumer_pid,*handle,*width,*height,true);
}
// Trial-only final-map composition. The helper never receives or owns the game
// HWND; x86 imports this completed texture and remains the sole presenter.
extern "C" __declspec(dllexport) int c3x_renderer_trial_present_shared(
    c3x_renderer_gpu_present_v1 const* request,DWORD consumer_pid,
    std::uint64_t* handle,unsigned* width,unsigned* height){
    if(!request||request->struct_size!=sizeof(*request)||request->action<0||request->action>2||
...
extern "C" __declspec(dllexport) int c3x_renderer_gpu_camera_poll(
    c3x_renderer_i64 ticket,c3x_renderer_gpu_frame_v1* view,c3x_renderer_output_v1* metadata){
...
extern "C" __declspec(dllexport) int c3x_renderer_gpu_camera_poll_view(
    ...
    *result={sizeof(*result)};
    ...
    try{int code=remote_renderer_requested()?remote_renderer_backend()->images(*request,*result,readback,capacity):
        ...
        return input.result(code,[&](auto& out){out(result->image);out(result->pixel_count);
            ...
            if(witness){auto hash=c3x_renderer::asset_content_hash(reinterpret_cast<unsigned char const*>(readback),std::size_t(result->pixel_count)*4);for(auto part:hash)out(part);}});
    }
...
extern "C" __declspec(dllexport) void c3x_renderer_input_recording_finish(){c3x_inputs::runtime().finish();c3x_inputs::reset_canvas_capture();}

#include "input_recording/replay.h"
