...
public:
    ...
    void trim_to_prepared(c3x_renderer_benchmark_oracle_trim_v1 & result, bool oracle_limits=true) {
        ...
        while(!tile_geometry_cache.empty() && tile_geometry_cache_bytes>tile_geometry_runtime_budget) {
            ...
            release_geometry_vertex_buffers(oldest->second.buffers);resident_content.release(oldest->second.binding);tile_geometry_cache.erase(oldest);
    ...
    bool make_tile_cache_room(std::size_t bytes) {
        while (tile_geometry_cache_bytes + terrain_patch_index_bytes + bytes > tile_geometry_runtime_budget ||
            ...
            resident_content.release(oldest->second.binding);tile_geometry_cache.erase(oldest);
            ++frame_tiles_evicted;
    ...
    c3x_renderer::TileFootprint tile_footprint(CachedTileGeometry const & tile,
        ...
        for (auto const & layer : buffers)
            for (auto const & source_chunk : layer) {
                GeometryDrawRecord chunk=natural_world?project_natural_chunk(source_chunk,record):GeometryDrawRecord(source_chunk);
    ...
    c3x_renderer::fidelity::TerrainCompileInput terrain_compile_input(c3x_renderer_tile_v1 const& tile,
            c3x_renderer_frame_v1 const& frame,int ground,bool skip_shore,bool separate,bool indexed,bool heights) const {
        ...
        input.key={std::uint64_t(tile.tile_x),std::uint64_t(tile.tile_y),std::uint64_t(tile.real_terrain_type)*32+unsigned(ground),
            std::uint64_t(frame.tile_width),std::uint64_t(frame.tile_height),std::uint64_t(content_view_height),std::uint64_t(content_revision),
    ...
    bool render(c3x_renderer_frame_v1 const & frame, c3x_renderer_output_v1 & output,
        ...
        if(cpu_terrain_enabled && !prewarming){
            ...
            for(unsigned i=0;i<frame.tile_count;++i){
                ...
                auto input=terrain_compile_input(tile,frame,ground_type(tile),skip_flat_shore,separate_natural_relief,index_natural_grids,retain_height_samples);
        ...
        for (c3x_renderer_u32 preparation_slot = 0; preparation_slot < (batch_preparing?preparation_count:frame.tile_count); ++preparation_slot) {
            ...
            for (int edge = 0; edge < 4; ++edge) {
                auto found = topology_cache.current(observed_coordinate_key(
                    neighbor_coordinates[edge][0], neighbor_coordinates[edge][1]));
                relief_neighborhood = relief_neighborhood || (found != nullptr &&
                    (found->relief == 5 || found->relief == 6 || found->relief == 10));
            }
            if (pickup_profile) {
            ...
            mix_tile(content_view_width); mix_tile(content_view_height);
            ...
            if (route_assets_ready && (tile.road_mask != 0 || tile.railroad_mask != 0)) {
                ...
                for (int direction = 0; direction < 4; ++direction) {
                    ...
                    auto found = topology_cache.current(observed_coordinate_key(neighbor_x, neighbor_y));
                    if (found == nullptr)
                        continue;
            ...
            if(share_natural && !shared_hit && shared_natural!=tile_geometry_cache.end()){
                if(shared_natural->second.shared_natural && shared_natural->second.last_used!=tile_geometry_epoch){
                    ...
                    resident_content.release(shared_natural->second.binding);tile_geometry_cache.erase(shared_natural);
    ...
    int prepare_nearby_view(c3x_renderer_camera_request_v1 const& request) {
        ...
        if(ahead_active || area_pending){
            ...
            if(ahead_active && ahead_prospective && nearby_presented && frame.presentation_frequency>0 &&
               frame.presentation_time_ticks-publication.frame.presentation_time_ticks>=frame.presentation_frequency/10){
                if(!pending_refresh){
        ...
        if(nearby_presented && nearby.centered(frame) &&
           (nearby.map.output.visible_animation_count<=nearby.input.visible_animation_count || RendererState::resource_clock(frame)==RendererState::resource_clock(publication.frame)))
            return C3X_RENDERER_RESULT_OK;
        clear_ahead();
...
private:
    ...
    void retain_view(c3x_renderer::PreparedViewArea<PublishedMapFrame>& incoming) {
        ...
        if(nearby.map.output.bgra_pixels &&
        ...
        if(nearby.map.output.bgra_pixels && incoming.input.tile_width!=nearby.input.tile_width){
            for(auto it=retained_views.begin();it!=retained_views.end();) {
    ...
    void clear_ahead() {
        area_pending=false;ahead_prospective=false;
        for(auto& ready:ahead_ready)if(ready.output.bgra_pixels){++ahead_discarded;ready.clear();}
    ...
    void prepare_ahead(std::unique_lock<std::mutex>& lock) {
        ...
        if(area_pending) {
                ...
                if(built.prepare(ahead_frame,ahead_identity)) {
                    ...
                    if(ok && ahead_selected){
                        ...
                        ok=nearby.input.tile_width==built.input.tile_width && nearby.input.tiles &&
                            ...
                            nearby.project(ahead_selected_frame,ahead_identity,nearby.center,&output,ahead_frame.presentation_time_ticks);
                        if(ok)trim_views();
                        lock.unlock();
                    }else if(ok)ok=built.finish(output,renderer_state.prepared_view_dependencies());
                }
            ...
            if(!ok && ahead_prospective && ahead_cancelled.load(std::memory_order_relaxed) && prospective_views.size()<2){
            ...
            char detail[384];std::snprintf(detail,sizeof(detail),"ok=%u cancelled=%u width=%d height=%d bytes=%zu area_owners_cap=%zu selected=%u retained_views=%zu committed_bytes=%zu prepare_ms=%.3f",
                unsigned(ok),unsigned(ahead_cancelled.load()),ahead_frame.target_width,ahead_frame.target_height,
                nearby.bytes(),retained_view_budget+decltype(nearby)::budget,unsigned(ahead_selected),retained_views.size()+1,retained_view_bytes(),renderer_state.trace.milliseconds(end.QuadPart-begin.QuadPart));
    ...
    void run() {
        ...
        for (;;) {
            ...
            if(command==Command::render && result==C3X_RENDERER_RESULT_OK){
                ...
                start_ahead();
            }else if(command!=Command::unit){nearby.clear();retained_views.clear();prospective_views.clear();pending_refresh.reset();nearby_available=false;}
