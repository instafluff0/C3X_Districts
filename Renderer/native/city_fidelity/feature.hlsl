...
float3 q6_receiver_illumination(PixelInput input,float3 normal,
 ...
 return q8_local_irradiance(input.q6_world,normal,ambient_visibility)+frame_illumination(normal,q6_receiver_visibility(input,normal,legacy_shadow),ambient_visibility);
}
float3 q6_receiver_illumination(FeaturePixelInput input,float3 normal,
 ...
 return q8_local_irradiance(input.q6_world,normal,ambient_visibility)+frame_illumination(normal,visibility,ambient_visibility);
}
...
Q6SceneOutput Q8LegacyPSFeature(FeaturePixelInput input) { return q6_scene_output(q6_raw_feature(input)); }
...
{
    return Q8LegacyPSFeature(input).color;
}
...
Q6SceneOutput Q8_CITY_FEATURE_ENTRY(FeaturePixelInput p) {
 ...
 float metalness=0;
#if Q8_CITY_EXTRA_MATERIALS
 if(CityMaterialFlags.x>.5 && (channels&16)!=0)metalness=q8_surface_sample(resource_base_texture_4,p.uv,repeat_uv).r;
#endif
 ...
 if((channels&8)!=0) {
  ...
  if(p.q6_world.w>.5 && Q6ShadowFlags.x>.5)
   visibility=q6_world_visibility(pickup_shadow_feature,p.q6_world,n,false);
#endif
  if(CityMaterialFlags.x>.5)lit+=q8_city_environment_specular(n,roughness,base,metalness,ao);
