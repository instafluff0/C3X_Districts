...
#endif
 return q8_local_irradiance(input.q6_world,normal,ambient_visibility)+frame_illumination(normal,legacy_shadow,ambient_visibility);
}
...
float4 q3_natural_water(PixelInput input) {
 ...
 float2 reflected_uv=(input.position.xy+NativeReflectionTarget.zw)/Q3_REFLECTION_SIZE;
...
PixelInput VSReflection(IntegratedVertexInput input) {
 ...
 float base=project_world_content(input.position,input.q6_world.xyz,c3x_content_projection,c3x_viewport_translation_padding).y+h*NativeReflection.x;
