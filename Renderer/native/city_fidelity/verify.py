...
OUT=ROOT/'Renderer/verification/city_fidelity'
def main():
    ...
    env={'C3X_RENDERER_VISUAL_PROFILE':'' if a.profile=='default' else a.profile,'C3X_RENDERER_TRACE':'2',
        'C3X_RENDERER_TRACE_FILE':f'..\\verification\\city_fidelity\\{name}.log',
    ...
    scene='..\\verification\\pickup\\world.csv';cx,cy=69,50
    if a.synthetic:
        rows=[f'{x},{y},2,2,0,0,0' for y in range(20) for x in range(y%2,20,2)]
        (OUT/'dry-city.csv').write_text('C3X_BIQ_TERRAIN_V3,20,20,200\n'+'\n'.join(rows)+'\n')
        scene='..\\verification\\city_fidelity\\dry-city.csv';cx=cy=10
    cmd=' && '.join(f'set "{k}={v}"' for k,v in env.items())
    cmd+=f' && build\\biq_preview.exe build\\candidate\\C3XRenderer.dll ..\\.. ..\\default.custom_rendering.txt {scene} ..\\verification\\city_fidelity\\{name}.bmp 640 480 {cx} {cy} {a.zoom} {a.hour}'
