...
struct LinearOutput {float4 radiance:SV_Target0;float validity:SV_Target1;};
LinearOutput PSMain(P p) {
 ...
 LinearOutput result;result.radiance=float4(max(0,albedo*light),1);result.validity=1;return result;
}
LinearOutput PSFeature(P p){return PSMain(p);}
