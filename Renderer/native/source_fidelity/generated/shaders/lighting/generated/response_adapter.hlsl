...
{
    return l13a_layout > 0.5 ? q6_display_linear(color, 1.0) : color;
}
...
float4 PSFeature(FeaturePixelInput input) : SV_TARGET
{
    ...
    float3 display_color = q6_srgb_encode(saturate(frame_tone_map(
...
float4 PSMain(PixelInput input) : SV_TARGET
{
    ...
    if (input.panel < 0.5 && lab_mode > 5.5)
    {
        ...
        return float4(q6_srgb_encode(saturate(coast_source * exposure)), 1.0);
    }
    ...
    if (input.panel > 0.5 && input.surface_kind > 8.5 && input.surface_kind < 9.5)
    {
        ...
        return float4(q6_srgb_encode(saturate(frame_tone_map(
    ...
    if (input.panel > 0.5 && input.surface_kind > 5.5 && input.surface_kind < 6.5)
    {
        ...
        if (shoreline_integrated < 0.5)
        {
            ...
            return float4(q6_srgb_encode(float3(0.82, 0.87, 0.90)), legacy_alpha);
        }
        ...
        return float4(q6_srgb_encode(foam), foam_alpha);
    }
    if (input.panel > 0.5 && input.surface_kind > 4.5 && input.surface_kind < 5.5)
    {
        ...
        if (shoreline_integrated < 0.5)
        {
            ...
            return float4(q6_srgb_encode(saturate(legacy_water)), legacy_alpha);
        }
    ...
    if (input.panel > 0.5 && input.surface_kind > 3.5 && input.surface_kind < 4.5)
    {
        ...
        if (shoreline_integrated < 0.5)
        {
            ...
            return float4(q6_srgb_encode(saturate(legacy_bed * exposure)), 1.0);
        }
