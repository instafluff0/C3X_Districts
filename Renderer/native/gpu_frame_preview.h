static_assert(sizeof(c3x_renderer_gpu_frame_v1)==44 && sizeof(c3x_renderer_gpu_command_v1)==64 &&
...
if(ok && !std::strcmp(gpu_frame_test,"1")) {
    ...
    if(!gpu_render||!gpu_images)return 1;
    ...
    // mirror left by GPU-only rendering. It also retires the composition session.
    if(ok){c3x_renderer_output_v1 cpu={C3X_RENDERER_API_VERSION,sizeof(cpu)};
        ...
        verify_gpu(execute(stale)==C3X_RENDERER_RESULT_SUPERSEDED,"CPU mode retires GPU ticket");}
