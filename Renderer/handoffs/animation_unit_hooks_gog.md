...
capture/forwarding wrappers. The existing CSV rows are callable definitions, so
none of these wrappers is active yet. The project reserves CSV edits for the
human; no CSV file was modified by this work.

For the **GOG Windows VM prototype**, change these three existing rows to the
...
- `verification/animation/unit-injected-compile.json` covers the approved
  `TEST_INJECTED_CODE_COMPILE.bat` smoke. The current wrappers compile while
  still inactive; the updated inleads must be compiled again after the CSV edit.
...
Do not run the game yet on the basis of this CSV pickup alone. The animation
candidate has not been staged; resource facing and the combined checkpoint are
still being finalized. The final user test continues to use normal `INSTALL.bat`
with `enable_custom_rendered_units` controlling optional custom unit bodies.
