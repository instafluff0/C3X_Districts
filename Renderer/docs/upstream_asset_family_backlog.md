...
| Barbarian camps | Exact `IMPROVEMENT_BARBARIAN_CAMP -> LM_BARBARIAN_CAMP` chain; primitive `VIL_BAR_01` and optional later `VIL_BAR_IND` roots recursively normalize, with viewer-conditioned Civ III presence, tribe-stable composition, neutral color, separate units, resource coexistence, and authored-vs-source night policy frozen | A dedicated modular Lab v2 family gate; completed L19A hut/colony approval does not cover camps |
