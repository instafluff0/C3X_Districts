# Retained renderer experiments — historical record

This archive preserves prior protocols and findings. Embedded next-step instructions
are superseded by [the active workflow](../benchmark_workflow.md). No new tests
were run to archive these records.


Everything below is retained evidence/context. Its embedded priorities, approval
scope and phrases such as "current target", "next gate" or "before proceeding"
are historical statements, not continuation instructions. Do not append a new
active plan at the bottom; update the current status above and the single linked
workflow. Preserve measured findings and failed hypotheses for reuse.
...
## Historical continuation checkpoint — 2026-09-10
...
## Focused static gate result — 2026-09-10

Fresh Windows measurements used the dense 100x100 world, 2240x1192 output,
width-128 tiles, the fixed six-object fixture plus city/improvement/road/farm/
mine/resource stress objects, a four-column resident scroll, city fidelity,
waves/reflections disabled, retained world backdrops enabled, and 128-pixel
world regions. The same-build full control published exact ownership with zero
fallback/recovery and about 1.83 GiB of contiguous free address space, but took
718.1 ms total: 268.1 ms geometry, 60.0 ms draw submission and 165.8 ms
GPU/readback completion. This is still far above the 100 ms gate.

The causal ablations were narrower. The fixed six-object fixture alone took
784.3 ms with no visible animation; removing only its city did not help
(767.3 ms), while removing only its route/rail data reduced the same fixture to
393.2 ms (198.4 ms geometry, 118.8 ms readback). Removing dense resources from
the full fixture reduced the scroll to 522.8 ms and removed visible animation.
These runs retain exact scroll ownership, zero fallback/recovery, and more than
1.9 GiB of largest free region. They identify route/object composition and the
animated-resource backdrop pass as the high-value retained-layer targets, not
water effects.

The conservative draw-candidate index reduced bounds tests by 98% without
changing pixels, but measured 972.4 ms and was removed. The 2240x256 strip
experiment took 2876.2 ms and changed pixels, so it was removed. The existing
animation readback atlas measured 875.4 ms versus 864.3 ms and was not
promoted. A route-only reuse of that index was also removed: nested backdrop
submissions changed pixels and rose to 1,472.2 ms total / 310.0 ms draw
submission. The existing directional prefetch replay completed exact return
parity, but reported zero prepared tiles/blocks and left jumps at 568--1,019 ms;
the foreground was compiling the same prefetch records that the worker later
reported as prepared. A new opt-in `C3X_RENDERER_PREFETCH_FOREGROUND_CONTROL=2`
experiment retained a conservative two-tile dependency guard in the foreground
and moved only farther records to the existing worker. On a fresh identical
four-column scroll it preserved the exact bitmap and ownership while reducing
geometry from 257.3 to 228.6 ms and total time from 703.8 to 678.0 ms; fallback
and recovery stayed at zero and the largest free region stayed above 1.9 GiB.
After an explicit worker lead, replay prepared 176 tiles and reduced the +4,
+8, and diagonal jumps to 707.5, 828.2, and 785.8 ms, with exact return parity.
This is a real convergence rung, but it remains far above the 100 ms gate.
The remaining dominant cost is synchronous strip/background GPU completion plus
route/object draw submission, so no 24/64-unit, effects or native-presentation
rung is supported by this gate yet.

**Current implementation target:** retain the opt-in two-tile prefetch guard as
a reproducible diagnostic, then test one benchmark-only route/object layer split
on top of the existing retained front. Keep terrain and route/object ownership
separate, allow directional preparation to compile the next exposed route strip
off the foreground path, and publish only after the current-camera pixels and
ownership are exact. Measure route compilation, static-strip composition,
animated-resource backdrop work, and publication separately; require no
completed-map reuse, zero fallback/recovery, and >=512 MiB contiguous VA. Keep
action-driven unit poses on their own dirty cadence. Do not add another cache
tier or resume water-effect optimization until this layer experiment passes
below 100 ms, then below 33 ms.
