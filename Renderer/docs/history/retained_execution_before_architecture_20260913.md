# Historical retained renderer execution record

Preserved evidence from before the connected architecture implementation.
Current authority: [retained implementation](../retained_renderer_plan.md).
The task queues below are historical, not instructions.
...
## Current status — river work closed; complete shared-surface alternative selected
...
A complete shared-surface rendering alternative is selected next. Other gaps remain explicit dependencies,
...
### River/content validity evaluation — closed, rejected

The bounded prototype recorded river-page topology/height dependencies and removed
whole-map revisions from reusable ground/natural content keys. Two focused owner/
compiler tests and all four independent small edit redraws were exact. The matched
640x480 edit oracle rejected performance: visible edit/reversal took 2994.407 /
2945.933 ms versus retained control 1274.624 / 1246.590 ms. Distant edits were
19.512 / 19.631 ms versus 21.003 / 18.739 ms. Geometry preparation dominated the
regression; whole-page dependency propagation was too coarse for useful reuse.
No dense campaign was warranted. The candidate was removed and retained sources
restored; its source and complete immutable receipts remain under
`Renderer/native/build/local-content-validity-20260913/`, including `comparison.json`.
This closes an edit/rebuild evaluation, not an unchanged-map navigation result.
Do not expand the river task or substitute another nearby helper optimization.

### Single next implementation task — complete shared-surface alternative

**Next task:** build one bounded alternative through the existing renderer:
resident scene inputs → selected static/dynamic passes → a shared GPU color/depth
working surface → consolidated finishing/readback. Compare the complete path with
the retained renderer on representative stationary animation and scrolling; use
that result to select the architecture, rather than tuning individual helpers.

- **Capability afterward:** cache misses and new views can draw resident world
  content into one scene target, then accumulate dynamic content and finish/copy
  once. Regional reuse remains useful where it avoids work, but regional rendering
  must not be the mechanism for reconstructing the shared working surface.
- **Repeated work removed:** main-map versus animation backdrop redraws, repeated
  region pass setup, per-cell resolve/finishing, and the intervening main-map wait.
  Existing unchanged-map evidence separates 14 fully resident requests averaging
  64.48 ms from 14 requests with backdrop misses averaging 229.93 ms; these are
  cohorts, not an isolated causal saving. Both must remain visible in comparison.
- **Implementation boundary:** extend the existing worker, resident occurrences,
  shader/material passes and Glow/LinearTarget owners. Use one bounded scene target
  and explicitly account any immutable static snapshot needed for stationary reuse.
  Preserve common depth, material ordering, MSAA/filtering, current source content,
  visibility/ownership and single-threaded D3D responsibility. Partition selected
  draws by actual resource/pass limits if required; do not reconstruct screen cells.
- **Architectural acceptance:** independent small boundary, pose/removal, camera
  and zoom comparisons; account all target/snapshot/staging lifetimes, retain
  >=512 MiB sampled contiguous headroom and bounded shadow ownership. Changed pixels
  require diagnosis and Lab acceptance, never a silently weakened oracle.
- **Performance acceptance and stop:** run matched stationary animation and the
  existing dense unchanged-map scrolling sequence. Report total correct-result
  time, preparation, GPU command work, finishing, waits and CPU copy separately;
  the VM timestamp limitation remains explicit. Include newly exposed and resident
  views, target memory, reset and setup. Retain, revise within this complete path,
  or reject on these outcomes. No further helper/cache campaign or unrelated edits.
- **Destination connection:** selected resident passes and a coherent working
  surface provide the concrete GPU responsibility needed for broad batching and
  consolidated publication. Native consumption remains unidirectional: ready exact
  output is consumed on Civ III's next call. Resolve the documented no-ready-image
  overlay/picking constraint before enabling general asynchronous camera handoff;
  this standalone comparison does not authorize staging/install/game launch.
