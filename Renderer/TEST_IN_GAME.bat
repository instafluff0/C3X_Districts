...
if "%~1"=="--check" (
  echo PASS renderer_test_launcher: installed checkout, renderer DLL, current renderer, trace, and RUN.bat route ready; game not launched.
  exit /b 0
)
echo Starting Civ III with current renderer diagnostics through OutputDebugStringA.
