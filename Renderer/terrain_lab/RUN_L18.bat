...
if not defined C3X_LAB_MINES set "C3X_LAB_MINES=..\packs\ImprovementsNormalized"
...
if not exist "%C3X_LAB_MINES%\mine_runtime.bin" goto missing
if not exist "%C3X_LAB_MINE_SCENARIO%" goto missing

for %%M in (mines_noon mines_night mines_zoom2 mines_no_mines mines_only) do (
  build\terrain_lab.exe "%C3X_LAB_PACK%" "terrain_lab.hlsl" "%C3X_LAB_OUTPUT%\terrain_beauty_l18_%%M.bmp" beauty_%%M 0.26 4.0 1.0 72 0.085 "%C3X_LAB_VEGETATION%" "%C3X_LAB_DECALS%" "%C3X_LAB_BIQ_VIEW%" "%C3X_LAB_TERRAIN_ELEMENTS%" "%C3X_LAB_SHORE_FEATURES%" "%C3X_LAB_ROUTE_STYLES%" "%C3X_LAB_ROAD_SCENARIO%" "%C3X_LAB_ROUTE_DOODADS%" "%C3X_LAB_RAILROAD_SCENARIO%" "%C3X_LAB_RESOURCES%" "%C3X_LAB_RESOURCE_SCENARIO%" "%C3X_LAB_CITIES%" "%C3X_LAB_CITY_ADJUNCTS%" "%C3X_LAB_CITY_SCENARIO%" "%C3X_LAB_MINES%" "%C3X_LAB_MINE_SCENARIO%"
  if errorlevel 1 goto failed
)
...
exit /b 1
:missing
echo terrain_lab: L18 requires normalized mine runtime bundle and deterministic mine scenario
popd
exit /b 1
