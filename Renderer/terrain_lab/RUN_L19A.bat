...
if not exist "%C3X_LAB_TILE_OBJECTS%\tile_object_runtime.bin" goto missing
if not exist "%C3X_LAB_TILE_OBJECT_SCENARIO%" goto missing

for %%M in (tile_objects_noon tile_objects_night tile_objects_zoom2 tile_objects_no_objects tile_objects_only) do (
  build\terrain_lab.exe "%C3X_LAB_PACK%" "terrain_lab.hlsl" "%C3X_LAB_OUTPUT%\terrain_beauty_l19a_%%M.bmp" beauty_%%M 0.26 4.0 1.0 72 0.085 "%C3X_LAB_VEGETATION%" "%C3X_LAB_DECALS%" "%C3X_LAB_BIQ_VIEW%" "%C3X_LAB_TERRAIN_ELEMENTS%" "%C3X_LAB_SHORE_FEATURES%" "%C3X_LAB_ROUTE_STYLES%" "%C3X_LAB_ROAD_SCENARIO%" "%C3X_LAB_ROUTE_DOODADS%" "%C3X_LAB_RAILROAD_SCENARIO%" "%C3X_LAB_RESOURCES%" "%C3X_LAB_RESOURCE_SCENARIO%" "%C3X_LAB_CITIES%" "%C3X_LAB_CITY_ADJUNCTS%" "%C3X_LAB_CITY_SCENARIO%" "%C3X_LAB_IMPROVEMENTS%" "%C3X_LAB_MINE_SCENARIO%" "%C3X_LAB_IMPROVEMENTS%" "%C3X_LAB_FARM_SCENARIO%" "%C3X_LAB_TILE_OBJECTS%" "%C3X_LAB_TILE_OBJECT_SCENARIO%"
  if errorlevel 1 goto failed
)
...
exit /b 1
:missing
echo terrain_lab: L19A requires normalized tile-object runtime and deterministic scenario
popd
exit /b 1
