...
if not exist "%C3X_LAB_SHORE_FEATURES%\shore_runtime.bin" goto missing_routes
...
if not exist "%C3X_LAB_RAILROAD_SCENARIO%" goto missing_routes

for %%M in (railroads railroads_zoom2 railroads_no_railroads railroads_only railroads_crossings) do (
  build\terrain_lab.exe "%C3X_LAB_PACK%" "terrain_lab.hlsl" "%C3X_LAB_OUTPUT%\terrain_beauty_l15_%%M.bmp" beauty_%%M 0.26 4.0 1.0 72 0.085 "%C3X_LAB_VEGETATION%" "%C3X_LAB_DECALS%" "%C3X_LAB_BIQ_VIEW%" "%C3X_LAB_TERRAIN_ELEMENTS%" "%C3X_LAB_SHORE_FEATURES%" "%C3X_LAB_ROUTE_STYLES%" "%C3X_LAB_ROAD_SCENARIO%" "%C3X_LAB_ROUTE_DOODADS%" "%C3X_LAB_RAILROAD_SCENARIO%"
  if errorlevel 1 goto failed
)
...
:missing_alternate
echo terrain_lab: L15 requires the local alternate environment terrain and vegetation packs
popd
exit /b 1

:missing_routes
echo terrain_lab: L15 requires normalized route/bridge packs and deterministic Lab road/railroad scenarios
popd
exit /b 1
