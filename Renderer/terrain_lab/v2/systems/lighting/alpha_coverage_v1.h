...
#include <cmath>
namespace q6 {
// Source-independent BC1/BC3/BC4 coverage for CPU light-depth preparation.
...
inline float alpha_nearest(labv2::Texture const& texture,float u,float v){
 ...
 if(texture.format==77||texture.format==78||texture.format==80){
   auto p=m.bytes.data()+(y/4)*m.pitch+(x/4)*16;
   if(texture.format==80)p=m.bytes.data()+(y/4)*m.pitch+(x/4)*8;
 ...
 throw std::runtime_error("Q6 alpha coverage requires declared BC1/BC3/BC4 texture");
}
}
