"""Freeze a source-selected, wholly unseen ground-material regression region."""
import json
...
def main():
    out=V2/'fixtures/beauty/surface-decals-foundation/freshground'
    ...
    for r in reg['regions']:
        if 'beauty' in r['id'] and r['id'] != 'beauty-freshground-100-v1':
    ...
    for x in range(0,100,2):
        for y in range(16,84,2):
            ...
            if c.get('grassland',0)<15 or c.get('plains',0)<15 or c.get('hills',0)<3:continue
    ...
    request={'source_sha256':reg['source']['sha256'],'regions':[{
        'requested_id':'beauty-freshground-100-v1','origin':[x,y],'extent':[10,10],
        'halo':6,'role':'user_evaluation','camera':{'viewport':[1360,800],'zooms':[1,2],'hours':[12,0]}}]}
    save(out/'region-request.json',request)
    existing=next((r for r in reg['regions'] if r['id']=='beauty-freshground-100-v1'),None)
    if existing:
    ...
    real_map.export('beauty-freshground-100-v1',out,'Q8-beauty',False)
    ...
    m['id']='surface-decals-foundation-freshground';f['modules']=[(out/'terrain.module.json').relative_to(ROOT).as_posix()]
    ...
    save(out/'BENCHMARKS.json',{'region':f['real_map']['region'],'projection':m['projection'],
        'selection':'Before viewing: max minimum wrapped origin distance among even-coordinate 10x10 crops with >=15 grassland, >=15 plains, >=3 hills and ALL 100 tiles outside prior beauty regions.',
