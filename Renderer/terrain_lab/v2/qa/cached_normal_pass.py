...
def main():
    ...
    p.add_argument('--region',choices=['coastal','inland','wilderness','freshground','freshcache'],required=True)
    p.add_argument('--ao',action='store_true')
    a=p.parse_args();region=a.region
    candidate='cached-normal-r2' if a.ao else 'cached-normal-r1'
    recipe='surface-decals-foundation' if region=='freshground' else 'river-corridor-r3'
    if region=='freshcache':recipe='cached-normal-foundation'
    baseline='surface-decals-foundation-v2' if region=='freshground' else recipe
    source=V2/'audits/beauty/out'/baseline/region
    inputs=V2/'audits/beauty/out'/(candidate+'-input')/region
    out=V2/'audits/beauty/out'/candidate/region
    ...
    normals=V2/'fixtures/beauty'/('source-normal-cache-r2' if a.ao else 'source-normal-cache-r1')
    ...
    fixture=V2/'fixtures/beauty'/candidate/region;fixture.mkdir(parents=True,exist_ok=True)
    shader=fixture/'combined.hlsl'
    shader.write_text('#define Q2_CACHED_NORMAL 1\n'+('#define Q2_CACHED_OCCLUSION 1\n' if a.ao else '')+f'#include "../../{recipe}/{region}/combined.hlsl"\n')
