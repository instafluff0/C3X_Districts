// Frozen v1 migration snapshot. See frozen_sources.json and MIGRATION.md.
#include <algorithm>
#include <cmath>
#include <cstdint>
#include <cstdio>
...
int lab_v2_continuous_desert = 0;
...
struct FeatureAsset {
    std::string id, source_texture_path;
...
struct FeatureBundle {
    ...
    std::string source_path,source_sha256;
};
...
// Optional normalized source override for Lab candidates; frozen default is 1.
float lab_v2_hill_height_multiplier = 1.0f;
...
bool load_biq_window(char const * path, BiqWindow & output) {
    ...
    bool ok = std::fscanf(file, "%39[^,],%d,%d,%u,%d,%d,%d,%d,%u\n", magic, &output.columns, &output.rows,
              ...
              halo_count <= static_cast<unsigned>(
                  16 * (output.columns + output.rows) + 256) &&
        ...
        for (unsigned index = 0; ok && index < halo_count; ++index) {
            ...
            int parsed = has_river_topology
                ...
                : std::fscanf(file, "%d,%d,%d,%d,%d,%d,%u,%u\n",
                           ...
                           &tile.base, &tile.real, &tile.bonus, &tile.overlays);
            if (parsed != (has_river_topology ? 9 : 8) ||
                tile.column < -8 || tile.column >= output.columns + 8 ||
                tile.row < -8 || tile.row >= output.rows + 8 ||
                (tile.column >= 0 && tile.column < output.columns &&
                 tile.row >= 0 && tile.row < output.rows) ||
                tile.base < 0 || tile.base > 13 || tile.real < 0 || tile.real > 13) {
                ok = false;
...
bool load_route_scenario(char const * path, char const * expected_magic,
    ...
    char provenance[40] = {};
    unsigned count = 0;
    bool ok = std::fscanf(file, "%39[^,],%d,%d,%u,%79[^,],%39[^\n]\n", magic, &output.columns,
                       &output.rows, &count, source_hash, provenance) == 6 &&
              std::strcmp(magic, expected_magic) == 0 &&
              std::strcmp(provenance, "lab_augmentation") == 0 &&
              output.columns > 0 && output.rows > 0 && count <= 4096;
    output.edges.reserve(count);
    for (unsigned index = 0; ok && index < count; ++index) {
        RoadEdge edge;
        ok = std::fscanf(file, "%d,%d,%d,%d,%u,%u,%u,%u\n",
...
bool load_resource_scenario(char const * path, ResourceScenario & output) {
    ...
    char provenance[40] = {};
    unsigned count = 0;
    bool ok = std::fscanf(file, "%47[^,],%d,%d,%u,%79[^,],%39[^\n]\n", magic, &output.columns,
                       &output.rows, &count, source_hash, provenance) == 6 &&
...
bool load_city_scenario(char const * path, CityScenario & output) {
    ...
    bool ok = std::fscanf(file, "%47[^,],%d,%d,%u,%79[^,],%39[^\n]\n", magic, &output.columns,
              ...
              std::strcmp(provenance, "lab_augmentation") == 0 &&
              output.columns > 0 && output.rows > 0 && count <= 128;
    output.instances.reserve(count);
    for (unsigned index = 0; ok && index < count; ++index) {
        CityInstance instance;
        ok = std::fscanf(file, "%d,%d,%u,%u,%u,%u,%u,%u,%u,%u\n",
...
bool load_mine_scenario(char const * path, MineScenario & output) {
    ...
    for (unsigned index = 0; ok && index < count; ++index) {
        ...
        ok = std::fscanf(file, "%d,%d,%u,%u,%u,%u\n", &instance.column, &instance.row,
...
bool load_farm_scenario(char const * path, FarmScenario & output) {
    ...
    bool ok = std::fscanf(file, "%47[^,],%d,%d,%u,%79[^,],%39[^\n]\n", magic, &output.columns,
              ...
              std::strcmp(provenance, "lab_augmentation") == 0 &&
              output.columns > 0 && output.rows > 0 && count <= 256;
    output.instances.reserve(count);
    for (unsigned index = 0; ok && index < count; ++index) {
        FarmInstance instance;
        ok = std::fscanf(file, "%d,%d,%u,%u,%u,%u,%u\n",
...
bool load_tile_object_scenario(char const * path, TileObjectScenario & output) {
    ...
    for (unsigned index = 0; ok && index < count; ++index) {
        ...
        ok = std::fscanf(file, "%d,%d,%u,%u,%u,%u,%u,%u,%u\n",
...
bool load_infrastructure_scenario(char const * path, InfrastructureScenario & output) {
    ...
    for (unsigned index = 0; ok && index < count; ++index) {
        ...
        ok = std::fscanf(file, "%d,%d,%u,%u,%u,%u,%u,%u\n",
                      &instance.column, &instance.row, &instance.kind,
                      &instance.variant, &instance.era, &instance.owner,
...
bool load_unit_scenario(char const * path, UnitScenario & output) {
    ...
    char provenance[64] = {};
    unsigned count = 0;
    bool ok = std::fscanf(file, "%47[^,],%d,%d,%u,%79[^,],%63[^\n]\n", magic, &output.columns,
                       &output.rows, &count, source_hash, provenance) == 6 &&
    ...
    for (unsigned index = 0; ok && index < count; ++index) {
        ...
        ok = std::fscanf(file, "%d,%d,%u,%u,%u,%u,%u,%u,%u,%d,%d,%u\n",
                      &instance.column, &instance.row, &instance.kind,
...
bool load_feature_bundle(std::string const & path, FeatureBundle & output) {
    ...
    output.source_path=labv2::portable_source(path);output.source_sha256=labv2::content_sha256(data.data(),data.size());
    std::size_t cursor = 8;
    ...
    for (FeatureAsset & asset : output.assets) {
        ...
        if (!consume_string(data, cursor, asset.id) ||
            ...
            return false;
        asset.source_texture_path=output.texture_paths.at(asset.texture_index);
...
bool load_dds(ID3D11Device * device, std::string const & path, DXGI_FORMAT expected,
    ...
    if(SUCCEEDED(hr)) {
        auto portable=labv2::portable_source(path);std::ostringstream meta;
        meta<<"{\"path\":"<<labv2::json_string(portable)<<",\"sha256\":"<<labv2::json_string(labv2::content_sha256(dds.data(),dds.size()))<<",\"packet_texture\":"<<(*output)->id<<",\"source_format\":"<<format<<",\"view_format\":"<<expected<<",\"width\":"<<width<<",\"height\":"<<height<<",\"mips\":"<<mip_count<<"}";
        labv2::source_textures[portable+"#"+std::to_string((*output)->id)]=meta.str();
    }
    release(texture);
    if (FAILED(hr))
...
bool load_r8_height(ID3D11Device * device, std::string const & path, DXGI_FORMAT source_format,
    ...
    if(SUCCEEDED(hr)) {
        auto portable=labv2::portable_source(path);std::ostringstream meta;
        meta<<"{\"path\":"<<labv2::json_string(portable)<<",\"sha256\":"<<labv2::json_string(labv2::content_sha256(dds.data(),dds.size()))<<",\"packet_texture\":"<<(*output)->id<<",\"source_format\":"<<source_format<<",\"view_format\":"<<desc.Format<<",\"width\":"<<field.width<<",\"height\":"<<field.height<<",\"mips\":"<<1<<"}";
        labv2::source_textures[portable+"#"+std::to_string((*output)->id)]=meta.str();
    }
    release(texture);
    if (FAILED(hr))
...
float dune_region_weight(float world_x, float world_y) {
    ...
    if (biq_scene_enabled) {
        if (lab_v2_continuous_desert) {
            // One C1 field for all incident tiles, including the skirt on the
            // neighboring biome. This fixes the ownership discontinuity; the
            // inherited dune body below remains an unapproved diagnostic.
            int left = static_cast<int>(std::floor(world_x - 0.5f));
            int top = static_cast<int>(std::floor(world_y - 0.5f));
            float sx = smoothstep01(world_x - 0.5f - left);
            float sy = smoothstep01(world_y - 0.5f - top);
            float weight = 0.0f;
            for (int y = 0; y != 2; ++y)
                for (int x = 0; x != 2; ++x) {
                    auto tile = biq_tile_at(left + x, top + y);
                    if (tile && tile->base == 0 && tile->real == 0)
                        weight += (x ? sx : 1-sx) * (y ? sy : 1-sy);
                }
            return weight;
        }
...
void biq_material_weights(BiqWindowTile const & tile, float u, float v,
    ...
    if (labv2::terrain_hooks.material_weights) {
        labv2::terrain_hooks.material_weights(world_x,world_y,weights);
        float sum=0;
        for(float w:weights) { if(!std::isfinite(w)||w<0) throw std::runtime_error("invalid material provider weight");sum+=w; }
        if(std::abs(sum-1)>0.001f) throw std::runtime_error("material provider weights must sum to one");
        return;
    }
...
float biq_signed_shore_distance(BiqWindowTile const & tile, float u, float v) {
    float world_x = static_cast<float>(tile.column) + u;
    float world_y = static_cast<float>(tile.row) + (1.0f - v);
    if(labv2::hydrology_hooks.signed_shore_distance) {
        float value=labv2::hydrology_hooks.signed_shore_distance(world_x,world_y);
        if(!std::isfinite(value)||value < -1 || value > 1)throw std::runtime_error("invalid hydrology shore provider");
        return value;
    }
    float grid_x = world_x - 0.5f;
    float grid_y = world_y - 0.5f;
...
float biq_tile_height(BiqWindowTile const & tile, float u, float v,
    ...
    if (lab_v2_continuous_desert) {
        // Keep triangles straddling the optical shoreline on the water datum.
        // A zero-width contour alone still interpolates displaced land vertices
        // across that contour, leaving sawteeth against the separate flat bed.
        float dune_coast = lab_v2_continuous_desert >= 3
            ? smoothstep01((-biq_signed_shore_distance(tile,u,v)-0.20f)/0.42f)
            : coastal_envelope;
        height += dune_height_value(world_x, world_y) * dune_coast *
            (lab_v2_continuous_desert >= 2 ? biq_hill_compatibility_envelope(tile,u,v) : 1.0f);
    }
...
Vertex make_biq_vertex(BiqWindowTile const & tile, float u, float v, float uv_scale,
    ...
    float height = biq_tile_height(tile, u, v, authored_height, authored_blend,
                                   flat_underlay);
    constexpr float step = 0.006f;
#ifdef LAB_V2_CONTINUOUS_NORMALS
    // Opt-in candidate: both incident faces sample the same world stencil.
    // The baseline below is retained verbatim for frozen v1 parity.
    auto normal = q4_relief::to_local_uv(q4_relief::continuous_normal(
        double(tile.column) + u, double(tile.row) + (1.0 - v),
        step, coast_projection.half_width * 2.0,
        [&](double x, double y) {
            return flat_underlay ? 2.5f : biq_world_height(
                float(x), float(y), authored_height, authored_blend);
        }));
    float slope_u = float(-normal[0] / normal[2]);
    float slope_v = float(-normal[1] / normal[2]);
#else
    ...
    if(labv2::hydrology_hooks.shore_sample) {
        ...
        for(float value:result.hydrology_data) if(!std::isfinite(value)) throw std::runtime_error("nonfinite hydrology sample");
    }
    if(labv2::terrain_hooks.material_uv && surface_kind<=1.0f) {
        float uv[2]={result.u,result.v};
        labv2::terrain_hooks.material_uv(tile.column+u,tile.row+(1-v),uv_scale,uv);
        if(!std::isfinite(uv[0])||!std::isfinite(uv[1])) throw std::runtime_error("invalid source material UV");
        result.u=uv[0];result.v=uv[1];
    }
    return result;
}
...
void add_feature_instance(FeatureBundle const & bundle, FeaturePlacement const & placement,
    ...
    {
        auto mesh_sha=labv2::content_sha256(asset.vertices.data(),asset.vertices.size()*sizeof(FeatureSourceVertex));
        auto index_sha=labv2::content_sha256(asset.indices.data(),asset.indices.size()*sizeof(uint32_t));
        std::vector<float> uv,normals;for(auto& v:asset.vertices){uv.insert(uv.end(),v.uv,v.uv+2);normals.insert(normals.end(),v.normal,v.normal+3);}
        std::ostringstream mesh;mesh<<"{\"id\":"<<labv2::json_string(asset.id)<<",\"bundle\":"<<labv2::json_string(bundle.source_path)<<",\"bundle_sha256\":"<<labv2::json_string(bundle.source_sha256)<<",\"vertices_sha256\":"<<labv2::json_string(mesh_sha)<<",\"indices_sha256\":"<<labv2::json_string(index_sha)<<",\"uv_sha256\":"<<labv2::json_string(labv2::content_sha256(uv.data(),uv.size()*4))<<",\"normal_sha256\":"<<labv2::json_string(labv2::content_sha256(normals.data(),normals.size()*4))<<",\"vertex_count\":"<<asset.vertices.size()<<",\"index_count\":"<<asset.indices.size()<<",\"base_color_source\":"<<labv2::json_string(labv2::portable_source(asset.source_texture_path))<<"}";
        labv2::source_meshes[mesh_sha]=mesh.str();
        std::ostringstream instance;instance<<std::setprecision(9)<<"{\"mesh_sha256\":"<<labv2::json_string(mesh_sha)<<",\"id\":"<<labv2::source_instances.size()<<",\"source_xy_anchor\":["<<world_x<<','<<world_y<<"],\"source_uniform_scale\":"<<scale<<",\"yaw_radians\":"<<rotation<<",\"ground_authoring_height\":"<<ground_height<<",\"projected_z_pixels_per_source_unit\":"<<feature_height_pixels_per_tile<<",\"ground_projection_vertical_scale\":"<<coast_projection.vertical_scale<<",\"world_z_authoring_divisor\":112,\"legacy_vertical_calibration_is_uniform_world_transform\":false}";
    ...
    for (std::size_t index = 0; index < asset.vertices.size(); ++index) {
        ...
        if(biq_tile) {
            transformed[index].world_x=biq_tile->column+(world_x-biq_tile->column)+local_x;
            transformed[index].world_y=biq_tile->row+1-(world_y-biq_tile->row+local_y);
            transformed[index].world_z=(ground_height+local_z*feature_height_pixels_per_tile/coast_projection.vertical_scale)/112.0f;
            transformed[index].world_valid=1;
        }
    }
    for (std::uint32_t index : asset.indices)
...
bool add_feature_group(FeatureBundle const & bundle, FeatureGroup const & group,
    ...
    for (unsigned instance = 0; instance < instance_count; ++instance) {
        ...
        if(labv2::placement_hooks.accept_vegetation && (group.name=="forest" || group.name=="jungle")) {
            ...
            float ground=biq_tile_height(*tile,world_x-std::floor(world_x),world_y-std::floor(world_y),authored_height,authored_blend);
            float c=std::cos(rotation),s=std::sin(rotation),height_pixels=promotion_scene_enabled?150.f:216.f;
            std::vector<float> xyz;
            for(auto& v:bundle.assets[placement->asset_index].vertices) {
                float x=world_x+(v.position[0]*c-v.position[1]*s)*scale;
                float y=tile->row+1-(world_y-tile->row+(v.position[0]*s+v.position[1]*c)*scale);
                xyz.push_back(64*(x+y));xyz.push_back(64*(x-y));
                xyz.push_back(64*(ground+v.position[2]*scale*height_pixels/coast_projection.vertical_scale)/112.f);
            }
            if(!labv2::placement_hooks.accept_vegetation(group.name.c_str(),bundle.assets[placement->asset_index].id.c_str(),seed,instance,xyz.data(),xyz.size()/3))continue;
        }
...
bool write_bmp(char const * path, D3D11_MAPPED_SUBRESOURCE const &, unsigned downsample) {
    if(std::getenv("C3X_LAB_V2_DOWNSAMPLE"))downsample=2;
    recorded.width=output_width; recorded.height=output_height; recorded.downsample=downsample;
    if (std::getenv("C3X_LAB_V2_LINEAR_SCENE")) {
        recorded.color_branch=1;
        recorded.valid_rect={0,0,output_width/downsample,output_height/downsample};
        std::vector<labv2::Draw> opaque,transparent;
        bool exposure_set=false;
        for(auto const& source:recorded.draws) {
            LabSettings settings;
            if(recorded.buffers[source.constant_buffer].size()!=sizeof(settings)) throw std::runtime_error("linear adapter constant layout drift");
            std::memcpy(&settings,recorded.buffers[source.constant_buffer].data(),sizeof(settings));
            float exposure=settings.exposure*(settings.l13a_layout>.5f?settings.environment_exposure:1.0f);
            if(exposure_set && std::abs(recorded.exposure-exposure)>1e-6f)throw std::runtime_error("per-category exposure forbidden in linear graph");
            recorded.exposure=exposure;exposure_set=true;
            // Copy before adding new packet buffers, which can reallocate the vector.
            auto bytes=recorded.buffers[source.vertex_buffer];
            std::vector<uint8_t> groups[2];
            for(unsigned vertex=0;vertex<source.count;vertex+=3) {
                bool blend=false;
                for(unsigned corner=0;corner<3;corner++) {
                    const uint8_t* raw=bytes.data()+size_t(vertex+corner)*source.stride;
                    bool value=false;
                    if(source.feature) {
                        if(source.stride!=sizeof(FeatureVertex))throw std::runtime_error("linear feature layout drift");
                        FeatureVertex v;std::memcpy(&v,raw,sizeof(v));
                        float fraction=v.material_index-std::floor(v.material_index);
                        value=v.material_index>=20.5f && v.material_index<28.5f && fraction>=.295f && fraction<.320f;
                    } else {
                        if(source.stride!=sizeof(Vertex))throw std::runtime_error("linear terrain layout drift");
                        Vertex v;std::memcpy(&v,raw,sizeof(v));
                        if(v.panel>.5f) {
                            if(v.surface_kind>=0 && v.surface_kind<=4)value=false;
                            else if(v.surface_kind>=5 && v.surface_kind<=14)value=true;
                            else throw std::runtime_error("unknown source surface kind in linear adapter");
                        }
                    }
                    if(corner && blend!=value)throw std::runtime_error("mixed state within source triangle");
                    blend=value;
                }
                auto& out=groups[blend?1:0];
                out.insert(out.end(),bytes.begin()+size_t(vertex)*source.stride,bytes.begin()+size_t(vertex+3)*source.stride);
            }
            for(unsigned group=0;group<2;group++)if(!groups[group].empty()) {
                auto d=source;
        ...
        recorded.draws=std::move(opaque);recorded.draws.insert(recorded.draws.end(),transparent.begin(),transparent.end());
        // Drop unused original geometry; preserve all referenced constant buffers.
        std::vector<int> indices(recorded.buffers.size(),-1);std::vector<std::vector<uint8_t>> buffers;
        for(auto& d:recorded.draws)for(auto ref:{&d.vertex_buffer,&d.constant_buffer}) {
            if(indices[*ref]<0){indices[*ref]=buffers.size();buffers.push_back(std::move(recorded.buffers[*ref]));}
            *ref=indices[*ref];
        }
        recorded.buffers=std::move(buffers);
    }
    // Migration structs carry opt-in attributes; do not bloat frozen wire2 payloads.
    std::map<unsigned,unsigned> packed_strides;
    for(auto& d:recorded.draws) {
        ...
        if(!packed_strides.count(d.vertex_buffer)) {
            ...
            packed.reserve(old.size()/d.stride*stride);
            for(size_t offset=0;offset<old.size();offset+=d.stride)packed.insert(packed.end(),old.begin()+offset,old.begin()+offset+stride);
            old=std::move(packed);packed_strides[d.vertex_buffer]=stride;
        } else if(packed_strides[d.vertex_buffer]!=stride)throw std::runtime_error("shared source vertex layout drift");
        d.stride=stride;
    }
    labv2::write_source_metadata(std::string(path)+".source.json",recorded);
    return labv2::write_packet(path,recorded);
}
bool compile_shader(char const *, char const * entry, char const *, ID3DBlob ** output) {
    *output=new ID3DBlob; (*output)->name=entry; return true;
}

std::string join(std::string const & root, char const * relative) {
    std::string result = root + "/" + relative;
...
int main(int argc, char ** argv) {
    ...
    territory_geometry_enabled = l21_mode && !beauty_complete_no_borders_mode &&
        std::getenv("C3X_LAB_V2_NO_TERRITORY") == nullptr;
    ...
    if (l11_scene_enabled && labv2::hydrology_hooks.initialize)
        labv2::hydrology_hooks.initialize(argv[12]);
    if (l11_scene_enabled && labv2::terrain_hooks.initialize)
        labv2::terrain_hooks.initialize(argv[12]);
    ...
    if (ok && promotion_mode) {
        ...
        const char* hill_override=std::getenv("C3X_LAB_V2_HILL_HEIGHT");
        if (const char* multiplier=std::getenv("C3X_LAB_V2_HILL_MULTIPLIER")) {
            lab_v2_hill_height_multiplier=std::atof(multiplier);
            if (!std::isfinite(lab_v2_hill_height_multiplier) ||
                lab_v2_hill_height_multiplier<=0 || lab_v2_hill_height_multiplier>4) return 2;
        }
        ok = load_r8_height(device,
                            hill_override ? hill_override : join(pack, "textures\\relief\\hills\\standard\\height_lod0.dds"),
    ...
    if (ok && (l10_mode || std::getenv("C3X_LAB_V2_COMPLETE_MATERIALS"))) {
        unsigned texture_width = 0;
        unsigned texture_height = 0;
    ...
    if (coast_mode) {
        ...
        if (const char* viewport = std::getenv("C3X_LAB_V2_VIEWPORT")) {
            unsigned width=0,height=0;
            if (std::sscanf(viewport,"%ux%u",&width,&height)!=2 || width<64 || height<64 || width>8192 || height>8192) return 2;
            output_width=width; output_height=height;
            float half_width=std::min((width-80.0f)/(biq_window.columns+biq_window.rows),
                                     (height-120.0f)*2.0f/(biq_window.columns+biq_window.rows));
            coast_projection={40.0f+half_width,60.0f+biq_window.rows*half_width*0.5f,half_width,half_width*0.5f,0.82f*half_width/112.0f};
        }
        // Lab gameplay crop: retain pixel-defined Civ III tile scale while
        // clipping a larger authoritative region to a filled viewport.
        if (const char* projection = std::getenv("C3X_LAB_V2_PROJECTION")) {
            float x=0,y=0,half=0;
        ...
        if (const char* query = std::getenv("C3X_LAB_V2_SURFACE_QUERY")) {
            FILE* input = std::fopen(query, "rb");
            FILE* output = std::fopen(argv[3], "wb");
            if (!input || !output) return 2;
            std::fprintf(output,"C3X_SURFACE_QUERY_V1,%u,%u,%.9g,%.9g,%.9g,%.9g,%.9g\n",output_width,output_height,coast_projection.origin_x,coast_projection.origin_y,coast_projection.half_width,coast_projection.half_height,coast_projection.vertical_scale);
            int column,row;float u,v; unsigned count=0;
            while (std::fscanf(input,"%d,%d,%f,%f\n",&column,&row,&u,&v)==4) {
                auto tile=biq_tile_at(column,row);
                if (!tile || !std::isfinite(u) || !std::isfinite(v) || u<0 || u>1 || v<0 || v>1 || ++count>65536) return 2;
                auto h=beauty_relief_enabled?&authored_height:nullptr;
                auto b=beauty_relief_enabled?&authored_blend:nullptr;
                auto vertex=make_biq_vertex(*tile,u,v,uv_scale,0,h,b);
                float height=biq_tile_height(*tile,u,v,h,b,false);
                std::fprintf(output,"%d,%d,%.9g,%.9g,%.9g,%.9g,%.9g,%.9g,%.9g,%.9g,%.9g,%.9g,%d,%d\n",column,row,u,v,height,(vertex.x+1)*output_width*.5f,(1-vertex.y)*output_height*.5f,vertex.terrain_depth,vertex.normal_x,vertex.normal_y,vertex.normal_z,vertex.shore_distance,tile->base,tile->real);
            }
            bool ok=std::feof(input) && count>0;
            std::fclose(input);std::fclose(output);return ok?0:2;
        }
    ...
    if (ok) {
        ...
        if (feature_geometry_enabled && feature_vertex_buffer != nullptr) {
            if (!biq_scene_enabled)
                context->ClearDepthStencilView(feature_depth_view, D3D11_CLEAR_DEPTH, 1.0f, 0);
            context->OMSetRenderTargets(1, &render_target, feature_depth_view);
            context->OMSetDepthStencilState(feature_depth_state, 0);
        ...
        if (farm_geometry_enabled && farm_vertex_buffer != nullptr) {
            context->OMSetRenderTargets(1, &render_target, feature_depth_view);
            context->OMSetDepthStencilState(feature_depth_state, 0);
            UINT feature_stride = sizeof(FeatureVertex);
            UINT feature_offset = 0;
        ...
        if (tile_object_geometry_enabled && tile_object_vertex_buffer != nullptr) {
            context->OMSetRenderTargets(1, &render_target, feature_depth_view);
            context->OMSetDepthStencilState(feature_depth_state, 0);
            UINT feature_stride = sizeof(FeatureVertex);
            UINT feature_offset = 0;
        ...
        if (damage_geometry_enabled && ground_state_vertex_buffer != nullptr) {
            context->OMSetRenderTargets(1, &render_target, feature_depth_view);
            context->OMSetDepthStencilState(feature_depth_state, 0);
            UINT feature_stride = sizeof(FeatureVertex);
            UINT feature_offset = 0;
        ...
        if ((fortification_geometry_enabled || airfield_geometry_enabled ||
            strategic_geometry_enabled) && fortification_vertex_buffer != nullptr) {
            context->OMSetRenderTargets(1, &render_target, feature_depth_view);
            context->OMSetDepthStencilState(feature_depth_state, 0);
            UINT feature_stride = sizeof(FeatureVertex);
            UINT feature_offset = 0;
        ...
        if (airfield_geometry_enabled && airfield_vertex_buffer != nullptr) {
            context->OMSetRenderTargets(1, &render_target, feature_depth_view);
            context->OMSetDepthStencilState(feature_depth_state, 0);
            UINT feature_stride = sizeof(FeatureVertex);
            UINT feature_offset = 0;
        ...
        if (city_geometry_enabled && city_vertex_buffer != nullptr) {
            context->OMSetRenderTargets(1, &render_target, feature_depth_view);
            context->OMSetDepthStencilState(feature_depth_state, 0);
            UINT feature_stride = sizeof(FeatureVertex);
            UINT feature_offset = 0;
        ...
        // The CPU recording adapter already owns submitted bytes. Release construction
        // capacity before packet packing to bound peak memory on the full fixture.
        std::vector<Vertex>().swap(vertices);
        for(auto list:{&feature_vertices,&city_vertices,&wall_vertices,&mine_vertices,
                      &farm_vertices,&tile_object_vertices,&fortification_vertices,
                      &airfield_vertices,&ground_state_vertices})std::vector<FeatureVertex>().swap(*list);
        for(auto& list:unit_vertices)std::vector<FeatureVertex>().swap(list);
