...
float3 q8_local_irradiance(float4 world,float3 normal,float ambient_visibility) {
 if(world.w<.5 || environment_night_activation<=0 || Q8_LOCAL_LIGHT_GAIN<=0)return 0;
 ...
 return light_sum*(Q8_LOCAL_LIGHT_GAIN*environment_night_activation*environment_emissive_scale*ambient_visibility);
}
