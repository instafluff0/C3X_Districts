...
cbuffer OutputSettings : register(b0) { float exposure;float gain;float2 inverse_scene; };
...
float4 PSOutput(float4 position : SV_Position) : SV_Target {
 ...
 float4 dynamic_color=moving.Load(at);
 float4 c=dynamic_color+scene.Load(at)*(1-dynamic_color.a);
 if(c.a<=.000001) return 0;
 c.rgb+=bloom.SampleLevel(linear_clamp,(position.xy+4.5)*inverse_scene,0).rgb*gain*c.a;
 ...
 return float4(saturate(rgb),saturate(c.a));
    ...
    bool draw(ID3D11RenderTargetView* target,int width,int height) {
        ...
        float values[4]={renderer.display_exposure,sandbox_fresh.glow.gain,
...
extern "C" __declspec(dllexport) int c3x_sandbox_present(HWND window,
    ...
    if(flat_present) {
        ...
        if(FAILED(renderer.device->CreateRenderTargetView(back,nullptr,&target))) {
            back->Release();return 5;
        }
        ...
        target->Release();
        if(!composed){back->Release();return 6;}
    }
    ...
    if (sequence_capture) {
        sprintf_s(capture_path,"%s-%04u.bmp",sequence_prefix,sequence_frame++);
    }
        ...
        if (SUCCEEDED(renderer.device->CreateTexture2D(&description, nullptr, &staging))) {
            ...
            if (SUCCEEDED(renderer.context->Map(staging, 0, D3D11_MAP_READ, 0, &mapped))) {
                ...
                if (!fopen_s(&file, capture_path, "wb") && file) {
                    ...
                    std::printf("CLIENT_CAPTURE result=%d time_ms=%lld path=%s\n",
