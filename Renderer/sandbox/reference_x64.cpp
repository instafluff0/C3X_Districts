...
int run_preview_case(int argc, char ** argv, HMODULE shared_module=nullptr, bool configure=true, bool bootstrap=false) {
    ...
    char full_world_option[8]={};
    bool capture_whole_world=GetEnvironmentVariableA("C3X_SANDBOX_WHOLE_WORLD",
        full_world_option,sizeof(full_world_option)) && full_world_option[0]=='1';
    ...
    for (CsvTile const & source : source_tiles) {
      for (int wrap_copy = -1; wrap_copy <= 1; ++wrap_copy) {
        if(capture_whole_world &&
            ((wrap_copy<0 && source.x<map_width-8) ||
             (wrap_copy>0 && source.x>=8))) continue;
        ...
        if(!capture_whole_world && pickup && (anchor_x+tile_width<0 || anchor_x>target_width || anchor_y+tile_height<0 || anchor_y>target_height))
            tile.tile_flags=C3X_RENDERER_TILE_TOPOLOGY_HALO|C3X_RENDERER_TILE_PREFETCH;
        if(!capture_whole_world && pickup && (anchor_x+tile_width < -tile_width*(prepared_view_fixture?8:4) || anchor_x>target_width+tile_width*(prepared_view_fixture?8:4) ||
