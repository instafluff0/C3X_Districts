...
struct Texture {
  ...
  std::vector<Mip> mips;
  uint32_t array_layers = 0; // wire7: zero is 2D; otherwise layer-major mip chains
  uint32_t mip_levels() const { return uint32_t(mips.size()) / (std::max)(1u, array_layers); }
};
struct Attribute {
...
struct Draw {
  ...
  // Wire8 source-shadow shader inputs: offsets into this same vertex buffer.
  uint32_t shadow_variant=0, shadow_material=UINT32_MAX, shadow_coverage=UINT32_MAX;
  std::vector<std::array<uint32_t,2>> shadow_textures; // shader slot, packet texture
  std::vector<Attribute> attributes;
  std::array<uint32_t, 128>
      textures{}; // 0 denotes explicit unbound/zero texture.
  std::array<uint32_t, 6> extra_constants{{UINT32_MAX,UINT32_MAX,UINT32_MAX,UINT32_MAX,UINT32_MAX,UINT32_MAX}};
  uint32_t constant(unsigned slot) const { return slot==0?constant_buffer:slot==1?frame_buffer:extra_constants.at(slot-2); }
};
struct WorldShadow {
  uint32_t shader_index=UINT32_MAX, texture=0, frame_buffer=UINT32_MAX, table_buffer=UINT32_MAX;
  uint32_t world_width=0,world_height=0,wrap_x=0,wrap_y=0;
  bool enabled() const{return shader_index!=UINT32_MAX;}
};
struct Packet {
  ...
  uint32_t binding_contract = 1; // 2: wire5 b1; 3: wire7 arrays; 4: wire8 world shadow pass
...
inline void transfer(Stream &s, Packet &p) {
  if(!s.reading && (p.color_branch>1 || p.binding_contract<1 || p.binding_contract>4 || p.geometry_contract>1 || (!p.color_branch && (p.binding_contract!=1 || p.geometry_contract))))throw std::runtime_error("unsupported packet extension contract");
  if(!s.reading && p.shadow.enabled() && p.binding_contract!=4)throw std::runtime_error("world shadows require wire8");
  uint32_t magic = 0x32514c43, version = p.color_branch ? (p.binding_contract==4?8:p.binding_contract==3?7:p.geometry_contract==1?6:p.binding_contract==2?5:p.shader_count>1?4:3) : packet_version;
  s.u32(magic);
  s.u32(version);
  if (magic != 0x32514c43 || version<1 || version>8)
  ...
  if (version >= 3) {
    ...
    if (version >= 7) p.binding_contract=3;
    if (version >= 8) p.binding_contract=4;
  ...
  if(version>=8){
    s.u32(p.shadow.shader_index);s.u32(p.shadow.texture);s.u32(p.shadow.frame_buffer);s.u32(p.shadow.table_buffer);
    s.u32(p.shadow.world_width);s.u32(p.shadow.world_height);s.u32(p.shadow.wrap_x);s.u32(p.shadow.wrap_y);
  }
  s.list(
      p.textures,
      [&](Texture &t) {
        ...
        s.u32(t.height);
        s.u32(t.format);
        if(version>=7)s.u32(t.array_layers);
        if(t.array_layers>256 || (t.array_layers && version<7))throw std::runtime_error("invalid texture array contract");
        ...
        s.list(
            ...
            15*(std::max)(1u,t.array_layers));
      },
      512);
  s.list(p.buffers, [&](std::vector<uint8_t> &b) { s.blob(b); }, version>=8?4096:256);
  s.list(
      p.draws,
      [&](Draw &d) {
        ...
        if (version >= 3) {
          ...
          if(version>=7)for(auto& buffer:d.extra_constants)s.u32(buffer);
          else for(auto buffer:d.extra_constants)if(buffer!=UINT32_MAX)throw std::runtime_error("extra constant buffers require wire7");
          if(version>=8){
            s.u32(d.shadow_variant);s.u32(d.shadow_material);s.u32(d.shadow_coverage);
            s.list(d.shadow_textures,[&](std::array<uint32_t,2>&b){s.u32(b[0]);s.u32(b[1]);},128);
          }
      ...
      version>=8?4096:1024);
  if(p.shadow.enabled()){
    auto const&w=p.shadow;
    if(p.geometry_contract!=1 || w.shader_index>=p.shader_count || !w.texture || w.texture>p.textures.size() ||
       p.textures[w.texture-1].format!=41 || !p.textures[w.texture-1].array_layers ||
       w.frame_buffer>=p.buffers.size() || p.buffers[w.frame_buffer].size()!=80 ||
       w.table_buffer>=p.buffers.size() || p.buffers[w.table_buffer].size()!=64*16 ||
       !w.world_width || !w.world_height || w.world_width>16384 || w.world_height>16384 ||
       w.wrap_x>1 || w.wrap_y>1)throw std::runtime_error("invalid world shadow pass");
  }
  for (auto &t : p.textures) {
    if (t.mips.empty() || t.mips.size()%(std::max)(1u,t.array_layers) || t.mip_levels()>15)
      throw std::runtime_error("missing texture mip chain");
    uint32_t w = t.width, h = t.height;
    unsigned level=0;
    for (auto &m : t.mips) {
      if(level++%t.mip_levels()==0){w=t.width;h=t.height;}
      ...
      if (t.format == 71 || t.format == 72 || t.format == 80) {
        ...
        pitch = w * 8;
      else if (t.format == 35 || t.format == 41 || t.format == 28 || t.format == 29 || t.format == 87 || t.format == 91)
        pitch = w * 4;
      else
  ...
  for (auto &d : p.draws) {
    ...
    if(version>=7)for(unsigned slot=0;slot<8;slot++) {
      unsigned buffer=d.constant(slot);
      if(buffer!=UINT32_MAX && (buffer>=p.buffers.size() || p.buffers[buffer].empty() || p.buffers[buffer].size()%16 || p.buffers[buffer].size()>65536))throw std::runtime_error("invalid constant buffer");
    }
    ...
    if(p.shadow.enabled() && (d.geometry_flags&1)){
      if(d.shadow_variant>1 || d.uv_attribute>=d.attributes.size() ||
         d.shadow_material>d.stride-4 || d.shadow_coverage>d.stride-4)
        throw std::runtime_error("invalid shadow caster layout");
      std::array<bool,128> used{};
      for(auto b:d.shadow_textures){
        if(b[0]>=128 || b[1]>p.textures.size() || used[b[0]])throw std::runtime_error("invalid shadow texture binding");
        used[b[0]]=true;
      }
    }
  }
