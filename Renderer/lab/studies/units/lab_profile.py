...
def prepare():
    ...
    replace(body,'        context->UpdateSubresource(beauty_frame,0,nullptr,beauty,0,0);','''        if(found->studio_profile) {
            beauty[0]=-.2817181f;beauty[1]=-.5231908f;beauty[2]=.8049086f;beauty[3]=1;
    ...
    replace(body,'(x+y)*32*zoom-z*(150.f*128/224)*zoom;','''(x+y)*32*zoom-z*(150.f*128/224)*zoom;
                if(found->studio_profile) {
                    ...
                    constexpr float pixels=104.637819f; // Z projects to 150*128/224 pixels.
                    sx=float(pose.anchor_x-request.body_x)+(x+y)*.70710678f*pixels*zoom;
                    sy=float(pose.anchor_y-request.body_y)-((-x+y)*.40557979f+z*.81915204f)*pixels*zoom;
                }''')
    ...
    replace(body,'int2 cell=', 'int2 cell=',count=0) if False else None
    ...
    replace(shader,'float ao=channels.x>.5?', 'if(Quality.y>1.5)return float4(studio_unit_response(i,albedo,n,1-occluded),1);\n float ao=channels.x>.5?')
...
def comparison(images):
    ...
    for entry in images:
        ...
        for mode,left_row in [('before-after',0),('matched-material',1)]:
            panel=Image.new('RGB',(1240,920),(47,52,54));draw=ImageDraw.Draw(panel)
            ...
            for i,(slug,_,_) in enumerate(material.SUBJECTS):
                ...
                for side,source_row in enumerate((left_row,2)):
                    ...
                    panel.paste(crop,(side*620+10+col*200,45+row*420))
                    draw.text((side*620+15+col*200,320+row*420),slug,fill='white')
...
def run(cases=('sizing',),zooms=(128,192)):
    ...
    print('PASS integrated Lab studio comparison; production unchanged:',unchanged)
    return receipt
