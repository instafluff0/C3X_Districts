...
int main(int argc, char **argv) {
  try {
    ...
    if(p.shadow.enabled())throw std::runtime_error("World-shadow packet replay is not connected here; use the production D3D11 comparator");
    ...
    for (auto &t : p.textures) {
      ...
      d.Width = t.width;
      d.Height = t.height;
      d.MipLevels = t.mip_levels();
      d.ArraySize = std::max(1u,t.array_layers);
      ...
      D3D11_SHADER_RESOURCE_VIEW_DESC srv={};srv.Format=d.Format;
      srv.ViewDimension=D3D11_SRV_DIMENSION_TEXTURE2DARRAY;
      srv.Texture2DArray.MipLevels=d.MipLevels;srv.Texture2DArray.ArraySize=d.ArraySize;
      check(dev->CreateShaderResourceView(texture.Get(), t.array_layers?&srv:nullptr, &view),
            "texture view");
    ...
    for (size_t i = 0; i < p.buffers.size(); i++) {
      bool constant = false;
      for (auto &d : p.draws)for(unsigned slot=0;slot<8;slot++)
        if (d.constant(slot) == i)
          constant = true;
    ...
    for (auto &d : p.draws) {
      ...
      ID3D11Buffer* constants[8]={};
      for(unsigned slot=0;slot<8;slot++)if(d.constant(slot)!=UINT32_MAX)constants[slot]=buffers[d.constant(slot)].Get();
      ctx->PSSetConstantBuffers(0,8,constants);ctx->VSSetConstantBuffers(0,8,constants);
