...
MTLPixelFormat pixel_format(uint32_t f) {
  ...
  case 41: return MTLPixelFormatR32Float;
...
GPUScene upload(id<MTLDevice> dev, const Packet &p) {
  ...
  for (auto &t : p.textures) {
    std::string key = std::to_string(t.width) + ":" + std::to_string(t.height) +
                      ":" + std::to_string(t.format) + ":" + std::to_string(t.array_layers);
    for (auto &m : t.mips)
    ...
    auto d = [MTLTextureDescriptor
        texture2DDescriptorWithPixelFormat:pixel_format(t.format)
                                     width:t.width
                                    height:t.height
                                 mipmapped:t.mip_levels() > 1];
    d.mipmapLevelCount = t.mip_levels();
    if(t.array_layers){d.textureType=MTLTextureType2DArray;d.arrayLength=t.array_layers;}
    ...
    for (size_t i = 0; i < t.mips.size(); i++) {
      auto &m = t.mips[i];
      unsigned level=unsigned(i)%t.mip_levels(),layer=unsigned(i)/t.mip_levels();
      [texture replaceRegion:MTLRegionMake2D(0, 0, std::max(1u, t.width >> level),
                                             std::max(1u, t.height >> level))
                 mipmapLevel:level
                       slice:layer
                   withBytes:m.bytes.data()
                 bytesPerRow:m.pitch
               bytesPerImage:m.bytes.size()];
    }
...
int render_job(int argc, char **argv) {
  @autoreleasepool {
    try {
      ...
      if(argc>=3 && std::string(argv[1])=="--validate-shaders") {
        if(argc==3) {
          for(auto entry:{"VSMain","VSFeature","PSMain","PSFeature"})shader(dev,argv[2],entry);
        } else {
          for(int i=3;i<argc;i++) {
            check(std::regex_match(argv[i],std::regex("(VS|PS)[A-Za-z0-9_]*")),"invalid shader entry name");
            shader(dev,argv[2],argv[i]);
          }
        }
        std::printf("PASS %d Metal shader entries\n",argc==3?4:argc-3);return 0;
      }
      if (argc == 2 && std::string(argv[1]) == "--capabilities") {
        ...
        printf("],\"max_anisotropy\":16,\"max_buffer_bytes\":%llu,\"argument_"
               ...
               "unorm\",\"blend\":\"straight_alpha\",\"packet_read_versions\":[1,2,3,4,5,6,7],\"linear_target\":\"rgba16float_premultiplied\",\"validity_target\":\"r8unorm\",\"constant_buffer_slots\":8,\"texture_array_layers\":256}\n",
      ...
      std::vector<id<MTLBuffer>> arguments,vertex_arguments;
      ...
      for (auto &d : p.draws) {
        ...
        auto make_arguments=[&](Shader const& program) {
        id<MTLArgumentEncoder> encoder =
            program.ids.empty()
                ? nil
                : [program.fn newArgumentEncoderWithBufferIndex:0];
        auto args = [dev
            newBufferWithLength:std::max<NSUInteger>(1, encoder.encodedLength)
                        options:MTLResourceStorageModeShared];
        [encoder setArgumentBuffer:args offset:0];
        for (auto binding : program.ids) {
          ...
          else if (binding < 138 && d.constant(binding-130)!=UINT32_MAX)
            [encoder setBuffer:g.buffers[d.constant(binding-130)] offset:0 atIndex:binding];
          else
            throw std::runtime_error("missing or unsupported shader binding");
        }
        return args;
        };
        arguments.push_back(make_arguments(stage_ps[d.shader_index*2+d.feature]));
        vertex_arguments.push_back(make_arguments(stage_vs[d.shader_index*2+d.feature]));
      }
      }
      ...
      for (unsigned frame = 0; frame < repeats; frame++) {
        ...
        for (size_t i = 0; i < p.draws.size(); i++) {
          ...
          [e setVertexBuffer:vertex_arguments[pipeline_offset+i] offset:0 atIndex:0];
          ...
          for(unsigned slot=0;slot<8;slot++)if(d.constant(slot)!=UINT32_MAX)
            [e useResource:g.buffers[d.constant(slot)] usage:MTLResourceUsageRead stages:MTLRenderStageVertex|MTLRenderStageFragment];
