...
def compact_packet(path, store):
    ...
    with path.open("rb") as source, target.open("wb") as output:
        ...
        for _ in range(textures):
            for _ in range(3):
                u32()
            layers=u32() if version>=7 else 0
            if layers>256:raise ValueError("invalid texture layer count")
            mips = u32()
            if mips > 15*max(1,layers) or mips%max(1,layers):
                raise ValueError("invalid mip count")
            for _ in range(mips):
                u32()
                blob()
        buffers = u32()
        if buffers > (4096 if version>=8 else 256):
