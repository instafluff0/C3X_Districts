...
bool lab_verify_objects(c3x_renderer_frame_v1 const& frame, c3x_renderer_output_v1 const& output) {
    ...
    for(unsigned i=0;i<frame.tile_count && ok;++i) {
        ...
        if((resources || shadows || sites) && tile.resource_id>=0){
    ...
    if(sites) {
        unsigned expected=(camps?0u:C3X_RENDERER_TILE_CUSTOM_HUT_REPLACED)|
            (huts?0u:(C3X_RENDERER_TILE_CUSTOM_CAMP_REPLACED|C3X_RENDERER_TILE_CUSTOM_RESOURCE_REPLACED));
